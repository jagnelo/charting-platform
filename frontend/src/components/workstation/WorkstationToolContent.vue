...
function familyCanonicalRoleEvidenceLabel(coverage: BenchmarkFamilyCoverageState) {
  ...
  return `${familyEvidence} · Canonical role evidence: ${roles.map(role => {
    ...
    const readinessEvidence = ` · availability ${availability} · status ${role.status ?? 'not reported'} · history bars ${barHistory} · route ${route} · refresh ${refresh} · entitlement ${entitlement} · readiness reasons ${reasons}`
