...
const linkedTargets = computed(() => workspaceStore.activeTab?.windows.some(window => window.tool_type === 'chart' && window.instance_key !== props.sourceWindowKey && window.link_group === props.linkGroup) ?? false)
...
function copy(index: number) { const item = chartStore.indicators[index]; if (!item) return; for (const window of workspaceStore.activeTab?.windows ?? []) { if (window.tool_type !== 'chart' || window.instance_key === props.sourceWindowKey || window.link_group !== props.linkGroup) continue; const plots = Array.isArray(window.configuration.indicators) ? window.configuration.indicators : []; window.configuration.indicators = [...plots, { ...item, params: { ...item.params }, style: { ...item.style }, lockedTimeframes: item.lockedTimeframes ? [...item.lockedTimeframes] : item.lockedTimeframes }] }; workspaceStore.scheduleSnapshot() }
</script>
<style scoped>
