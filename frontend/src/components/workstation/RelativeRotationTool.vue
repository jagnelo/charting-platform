...
function publishConfiguration() {
  emit('configuration', { ...(props.configuration ?? {}), group_key: groupKey.value, benchmark: benchmark.value.toUpperCase(), timeframe: timeframe.value, lookback: lookback.value, tail_length: tailLength.value, as_of: asOf.value || null, adjusted: adjusted.value })
}
...
async function load() {
  ...
  error.value = ''
  try {
    const payload = await api.get<{ rows: Row[]; freshness?: string }>(`/analysis/groups/${encodeURIComponent(groupKey.value)}/relative-rotation`, { benchmark: benchmark.value.toUpperCase(), timeframe: timeframe.value, adjusted: adjusted.value, lookback: lookback.value, tail_length: tailLength.value, as_of: asOfTimestamp() })
...
watch([groupKey, benchmark, timeframe, lookback, tailLength, asOf, adjusted], () => { publishConfiguration(); if (lookback.value >= 2 && tailLength.value >= 1) void load() })
