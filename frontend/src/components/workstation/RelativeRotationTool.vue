<template>
  <section class="rotation-tool">
    <header><strong>Relative Rotation · {{ benchmark }}</strong><div class="rotation-tool__controls"><label>Universe <select v-model="groupKey" aria-label="Rotation universe"><option value="sp500-sectors">S&amp;P 500 sectors</option><option value="us-benchmarks">US benchmarks</option></select></label><label>Benchmark <input v-model.trim="benchmark" aria-label="Rotation benchmark" /></label><label>Timeframe <select v-model="timeframe" aria-label="Rotation timeframe"><option value="D1">Daily</option><option value="W1">Weekly</option><option value="MN">Monthly</option></select></label><label>Lookback <input v-model.number="lookback" aria-label="Rotation lookback" type="number" min="2" max="252" /></label><label>Tail <input v-model.number="tailLength" aria-label="Rotation tail length" type="number" min="1" max="100" /></label><label class="rotation-tool__adjusted"><input v-model="adjusted" aria-label="Rotation split adjusted" type="checkbox" /> Adjusted</label></div><small>Trend: ratio return over {{ lookback }} bars · Momentum: change in that trend{{ freshness ? ` · ${freshness}` : '' }}</small></header>
...
function publishConfiguration() {
  emit('configuration', { ...(props.configuration ?? {}), group_key: groupKey.value, benchmark: benchmark.value.toUpperCase(), timeframe: timeframe.value, lookback: lookback.value, tail_length: tailLength.value, adjusted: adjusted.value })
}
async function load() {
  ...
  error.value = ''
  try {
    const payload = await api.get<{ rows: Row[]; freshness?: string }>(`/analysis/groups/${encodeURIComponent(groupKey.value)}/relative-rotation`, { benchmark: benchmark.value.toUpperCase(), timeframe: timeframe.value, adjusted: adjusted.value, lookback: lookback.value, tail_length: tailLength.value })
...
watch([groupKey, benchmark, timeframe, lookback, tailLength, adjusted], () => { publishConfiguration(); if (lookback.value >= 2 && tailLength.value >= 1) void load() })
