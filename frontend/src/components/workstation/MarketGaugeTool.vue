<template>
  <section ref="gaugeRoot" class="market-gauge">
    <header class="market-gauge__controls">
      ...
      <span v-if="gauge" class="market-gauge__freshness" :data-freshness="gauge.freshness">{{ freshnessLabel }}</span>
    </header>
    <p v-if="error" class="market-gauge__error">{{ error }}</p>
    <p v-else-if="scansQuery.isPending.value" class="market-gauge__state">Loading saved scans…</p>
...
const freshnessLabel = computed(() => {
  const value = gauge.value?.freshness?.replace(/_/g, ' ')
  return value ? value[0].toUpperCase() + value.slice(1) : 'Unknown freshness'
})
...
<style scoped>
.market-gauge { display: grid; align-content: start; gap: 7px; height: 100%; padding: 7px; background: #11161b; color: #9aabb6; font: 10px "Segoe UI", Arial, sans-serif; }.market-gauge__controls { display: grid; grid-template-columns: minmax(0, 1fr) auto auto; gap: 4px; align-items: center; }.market-gauge select,.market-gauge button { min-width: 0; height: 20px; border: 1px solid #34434e; background: #172027; color: #d2dce3; font: inherit; }.market-gauge button { padding: 0 6px; cursor: pointer; }.market-gauge button:disabled { cursor: wait; opacity: .65; }.market-gauge__freshness { color: #a9c8a2; white-space: nowrap; text-transform: capitalize; }.market-gauge__freshness[data-freshness="stale"],.market-gauge__freshness[data-freshness="coverage_limited"] { color: #dfbd82; }.market-gauge__freshness[data-freshness="unavailable"] { color: #e99a9a; }.market-gauge__reading { display:flex; align-items:baseline; justify-content:space-between; }.market-gauge__reading b { color:#78b9e4; font-size:24px; font-weight:500; }.market-gauge p,.market-gauge small { margin:0; }.market-gauge__lineage,.market-gauge__warnings { color: #778994; }.market-gauge__error { color:#e99a9a; }.market-gauge__state { margin:0; }
</style>
