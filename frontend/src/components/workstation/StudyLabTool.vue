...
const negativeStreakSource = "closes = market.close()\nstreaks = stats.streaks(closes, 'negative')\nindices = [record['end_index'] for record in streaks['records']]\noutput.scalar('current_negative_streak', streaks['current'])\noutput.scalar('longest_negative_streak', streaks['longest'])\noutput.scalar('average_negative_streak', streaks['average'])\noutput.scalar('shortest_negative_streak', streaks['shortest'])\noutput.table('completed_negative_streaks', streaks['records'])\noutput.table('forward_returns', research.forward_returns(dataset, indices, [1, 5, 20]))\noutput.events('negative_streak_events', research.occurrences(dataset, indices, 'negative_close_streak'))\noutput.histogram('negative_streak_distribution', streaks['lengths'], 8, streaks['current'])"
...
const volatilityRegimeSource = "closes = market.close()\nreturns = [0] + [(closes[index] / closes[index - 1]) - 1 for index in range(1, len(closes))]\nrolling_volatility = stats.rolling(returns, 20, 'std')\nvalid_volatility = [value for value in rolling_volatility if value is not None]\nmedian_volatility = stats.median(valid_volatility) if valid_volatility else 0\nvolatility = [value if value is not None else 0 for value in rolling_volatility]\nregime = [1 if value is not None and value >= median_volatility else 0 for value in rolling_volatility]\noutput.series('realised_volatility_20', volatility)\noutput.series('high_volatility_regime', regime)\noutput.boolean('currently_high_volatility', bool(regime and regime[-1]))\noutput.histogram('volatility_distribution', volatility, 12, volatility[-1] if volatility else None)"
...
const breadthThrustSource = "breadth = research.breadth_thrust(dataset, 90)\noutput.scalar('breadth_coverage', breadth['coverage'])\noutput.scalar('percent_price_advancing', breadth['percent_price_advancing'] if breadth['percent_price_advancing'] is not None else 0)\noutput.scalar('percent_volume_advancing', breadth['percent_volume_advancing'] if breadth['percent_volume_advancing'] is not None else 0)\noutput.boolean('qualifies_90_90', breadth['qualifies'])\noutput.table('breadth_thrust_members', breadth['rows'])\noutput.table('breadth_thrust_exclusions', breadth['exclusions'])"
const breadthThrustHistorySource = "breadth = research.breadth_thrust_history(dataset, 90)\noutput.scalar('historical_sample_size', breadth['sample_size'])\noutput.scalar('current_coverage', breadth['coverage'])\noutput.scalar('current_qualifies_90_90', breadth['qualifies'])\noutput.series('percent_price_advancing_history', breadth['price_percentages'])\noutput.series('percent_volume_advancing_history', breadth['volume_percentages'])\nprice_history = breadth['price_percentages']\nvolume_history = breadth['volume_percentages']\noutput.histogram('price_participation_distribution', price_history, 10, price_history[-1] if price_history else None)\noutput.histogram('volume_participation_distribution', volume_history, 10, volume_history[-1] if volume_history else None)\noutput.events('breadth_thrust_occurrences', research.occurrences(dataset, breadth['qualifying_indices'], '90_90_breadth_thrust'))\noutput.table('breadth_thrust_history', breadth['rows'])\noutput.table('breadth_thrust_exclusions', breadth['exclusions'])"
const genericBreadthSource = "period = int(parameters.get('period', 200))\ncondition = {'kind': 'above_moving_average', 'params': {'period': period, 'average': parameters.get('average', 'sma'), 'comparator': 'above'}}\nsnapshot = research.breadth_condition(dataset, condition)\nhistory = research.breadth_condition(dataset, condition, True)\noutput.scalar('current_percentage', snapshot['percentage'] if snapshot['percentage'] is not None else 0)\noutput.scalar('current_pass_count', snapshot['pass_count'])\noutput.scalar('current_eligible_count', snapshot['eligible_count'])\noutput.series('percentage_history', [point['percentage'] for point in history['points']])\noutput.table('breadth_members', snapshot['rows'])\noutput.table('breadth_exclusions', snapshot['exclusions'])\noutput.table('historical_breadth', history['points'])"
const genericHighBreadthSource = "lookback = int(parameters.get('lookback', 252))\nthreshold = float(parameters.get('threshold', 0.01))\ncondition = {'kind': 'within_52_week_high', 'params': {'lookback': lookback, 'threshold': threshold, 'direction': 'high'}}\nsnapshot = research.breadth_condition(dataset, condition)\nhistory = research.breadth_condition(dataset, condition, True)\noutput.scalar('current_percentage', snapshot['percentage'] if snapshot['percentage'] is not None else 0)\noutput.scalar('current_pass_count', snapshot['pass_count'])\noutput.scalar('current_eligible_count', snapshot['eligible_count'])\noutput.series('percentage_history', [point['percentage'] for point in history['points']])\noutput.table('breadth_members', snapshot['rows'])\noutput.table('breadth_exclusions', snapshot['exclusions'])\noutput.table('historical_breadth', history['points'])"
const crossSectionalBreadthSource = "field = str(parameters.get('field', 'close'))\npercentile = float(parameters.get('percentile', 0.5))\noperator = str(parameters.get('operator', 'gte'))\ncondition = {'kind': 'percentile', 'target_scope': 'cross_sectional', 'params': {'field': field, 'percentile': percentile, 'operator': operator}}\nsnapshot = research.breadth_condition(dataset, condition)\nhistory = research.breadth_condition(dataset, condition, True)\noutput.scalar('current_percentage', snapshot['percentage'] if snapshot['percentage'] is not None else 0)\noutput.scalar('current_pass_count', snapshot['pass_count'])\noutput.scalar('current_eligible_count', snapshot['eligible_count'])\noutput.series('percentage_history', [point['percentage'] for point in history['points']])\noutput.table('breadth_members', snapshot['rows'])\noutput.table('breadth_exclusions', snapshot['exclusions'])\noutput.table('historical_breadth', history['points'])"
...
const promotableKind = computed<'scalar' | 'boolean' | 'series' | 'events' | 'range' | null>(() => {
...
const latestRangeCenterObservation = computed(() => {
  ...
  return latestRangeCenterValue(artifact)
})
...
function latestRangeCenterValue(artifact: Artifact): number | null {
  const center = rangeData(artifact)?.center
  if (!center) return null
  for (const value of center.slice().reverse()) {
    if (typeof value === 'number' && Number.isFinite(value)) return value
  }
  return null
}
...
async function promoteRangeCenterCondition(target: SeriesConditionTarget) {
  ...
  if (!runSource.value) {
    promotionStatus.value = 'The immutable source code version for this range study is unavailable.'
    return
  }
  promotionBusy.value = true
  promotionStatus.value = ''
  try {
    ...
    if (target === 'column') {
      promotionStatus.value = `Saved range center “${artifact.name}” as a thresholded Boolean column.`
      return
    }
    let scanId = cached.id
    if (typeof scanId !== 'number') {
    ...
    if (target === 'filter') promotionStatus.value = `Saved range center “${artifact.name}” as a thresholded watchlist filter.`
    else if (target === 'scan') promotionStatus.value = `Promoted range center “${artifact.name}” to a thresholded scan.`
    else if (target === 'gauge') promotionStatus.value = `Range center “${artifact.name}” is available as a thresholded Market Gauge.`
    else {
      await api.post('/alerts/screener', { screener_id: scanId, trigger_type: 'entered', repeat: true, notes: `Created from Study Lab range center run ${studyRun.id} (${artifact.name})` })
      promotionStatus.value = `Promoted range center “${artifact.name}” to a thresholded scan alert.`
    }
  } catch (cause: any) {
