...
async function load() {
  ...
  try {
    ...
    ;[alerts.value, indicatorAlerts.value] = await Promise.all([
...
<style scoped>
.alerts-tool { display: grid; grid-template-rows: auto minmax(0, 1fr); height: 100%; min-height: 0; background: #11161b; color: #c7d0d8; font: 10px "Segoe UI", Arial, sans-serif; }
.alerts-tool__create { display: grid; grid-template-columns: minmax(0, 1fr) 66px 34px; gap: 4px; padding: 5px; border-bottom: 1px solid #2c3740; }
...
.alerts-tool__list li { display: grid; grid-template-columns: minmax(0, 1fr) auto auto; align-items: center; gap: 5px; min-height: 26px; padding: 3px 6px; border-bottom: 1px solid #20282f; }
