...
export function readPopoutGeometry(style: Record<string, unknown> | null | undefined): PopoutGeometry {
  const candidate = style?.popout
  if (!candidate || typeof candidate !== 'object') return { ...DEFAULT_GEOMETRY }
  const value = candidate as Record<string, unknown>
  return {
    left: finite(value.left) ? Math.round(value.left) : DEFAULT_GEOMETRY.left,
    top: finite(value.top) ? Math.round(value.top) : DEFAULT_GEOMETRY.top,
    width: finite(value.width) ? Math.max(MIN_WIDTH, Math.round(value.width)) : DEFAULT_GEOMETRY.width,
    height: finite(value.height) ? Math.max(MIN_HEIGHT, Math.round(value.height)) : DEFAULT_GEOMETRY.height,
  }
}
