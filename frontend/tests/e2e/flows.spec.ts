...
test.describe('TC2000 workstation', () => {
  ...
  test('F8s-breadth-family-ratio — family breadth exposes role-aware cap and market relative strength', async ({ page, browserDiagnostics }) => {
    ...
    await breadth.getByRole('button', { name: 'Evaluate' }).click()
    await expect.poll(() => customBreadthRequests.length, { timeout: 10_000 }).toBeGreaterThan(1)
    expect(customBreadthRequests.at(-1)?.condition).toMatchObject({ kind: 'rsi', params: { threshold: 55 } })
    await condition.selectOption('volume_ratio')
    await breadth.getByLabel('Breadth volume ratio target').fill('1.5')
    await breadth.getByRole('button', { name: 'Evaluate' }).click()
    await expect.poll(() => customBreadthRequests.length, { timeout: 10_000 }).toBeGreaterThan(2)
    expect(customBreadthRequests.at(-1)?.condition).toMatchObject({ kind: 'volume_ratio', params: { threshold: 1.5 } })
    ...
    await breadth.getByRole('button', { name: 'Evaluate' }).click()
    await expect.poll(() => customBreadthRequests.length, { timeout: 10_000 }).toBeGreaterThan(3)
    expect(customBreadthRequests.at(-1)?.condition).toMatchObject({ kind: 'range', params: { lower: -0.1, upper: 0.1, inclusive: true } })
    await condition.selectOption('percentile')
    await breadth.getByLabel('Breadth percentile rolling window').fill('20')
    await breadth.getByLabel('Breadth percentile target').fill('0.8')
    await breadth.getByRole('button', { name: 'Evaluate' }).click()
    await expect.poll(() => customBreadthRequests.length, { timeout: 10_000 }).toBeGreaterThan(4)
    expect(customBreadthRequests.at(-1)?.condition).toMatchObject({ kind: 'percentile', params: { period: 20, percentile: 0.8, operator: 'gte' } })
    await browserDiagnostics.expectNoCriticalIssues()
  })
