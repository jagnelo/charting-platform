...
test.describe('TC2000 workstation', () => {
  ...
  test('F8s-breadth-family-ratio — family breadth exposes role-aware cap and market relative strength', async ({ page, browserDiagnostics }) => {
    ...
    await expect(breadth.getByLabel('Benchmark family canonical role evidence')).toContainText('Cap weight SPY · members 1 · weighted 1 (ready) · classified 1 (ready) · point-in-time supported · history ready · readiness ready', { timeout: 15_000 })
    await expect(breadth.getByLabel('Benchmark family canonical role evidence')).toContainText('Equal weight RSP · members 1 · weighted 1 (ready) · classified 1 (ready) · point-in-time supported · history ready · readiness ready', { timeout: 15_000 })
