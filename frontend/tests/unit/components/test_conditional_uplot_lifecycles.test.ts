...
import CrossFamilyRankHistoryUPlot from '@/components/workstation/CrossFamilyRankHistoryUPlot.vue'
...
const cases = [
  ...
  { name: 'cross-family rank history', component: CrossFamilyRankHistoryUPlot, valid: { history: { timeframe: 'D1', adjustment: 'split_adjusted', rank_period: '1M', limit: 500, rows: [{ family_key: 'sp500', family_name: 'S&P 500', official_index_symbol: 'SPX', symbol: 'SPY', label: 'SPY', available: true, coverage: 1, points: [{ timestamp: '2026-01-01', rank: 1, performance: {}, relative_performance: {} }], warnings: [] }], exclusions: [] } }, invalid: { history: { timeframe: 'D1', adjustment: 'split_adjusted', rank_period: '1M', limit: 500, rows: [{ family_key: 'sp500', family_name: 'S&P 500', official_index_symbol: 'SPX', symbol: 'SPY', label: 'SPY', available: true, coverage: 1, points: [{ timestamp: 'not-a-date', rank: null, performance: {}, relative_performance: {} }], warnings: [] }], exclusions: [] } } },
...
describe('conditional uPlot lifecycle contracts', () => {
  ...
  it('aligns cross-family ranks without forward-filling gaps', async () => {
    const wrapper = mount(CrossFamilyRankHistoryUPlot, {
      props: { history: { timeframe: 'D1', adjustment: 'split_adjusted', rank_period: '1M', limit: 500, rows: [
        { family_key: 'sp500', family_name: 'S&P 500', official_index_symbol: 'SPX', symbol: 'SPY', label: 'SPY', available: true, coverage: 1, points: [{ timestamp: '2026-01-01', rank: 1, performance: {}, relative_performance: {} }, { timestamp: '2026-01-02', rank: 2, performance: {}, relative_performance: {} }], warnings: [] },
        { family_key: 'nasdaq100', family_name: 'Nasdaq 100', official_index_symbol: 'NDX', symbol: 'QQQ', label: 'QQQ', available: true, coverage: 1, points: [{ timestamp: '2026-01-02', rank: 1, performance: {}, relative_performance: {} }], warnings: [] },
      ], exclusions: [] } },
    })
    await vi.waitFor(() => expect(vi.mocked(uPlot)).toHaveBeenCalledTimes(1))
    const call = vi.mocked(uPlot).mock.calls[0] as any
    expect(call[1][0]).toHaveLength(2)
    expect(call[1][1]).toEqual([1, 2])
    expect(call[1][2]).toEqual([null, 1])
    wrapper.unmount()
  })
