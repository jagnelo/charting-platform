...
- Current strict matrix after this native-route promotion batch: `496`
  registered / `338` native-live-backed / `158` fallback-only / target `496`
...
- Validation passed:
  ...
  - strict manifest recompute: `496` registered / `338` native /
    `158` fallback / gap `0`
...
- Continue native-route discovery for the remaining `158` audited
  fallback-only providers, prioritizing higher-AUM and higher-ETF-count
  providers with executable first-party complete holdings routes.
...
- Current strict matrix after this source-reconciliation batch: `450`
  registered / `332` native-live-backed / `118` fallback-only / target `496`
  promoters / `46` registered-promoter gap / `generated_recognition_only []`.
- This reduces the named source-reconciliation gap by another `10`; it does not
  add native routes for those providers yet.
- Implementation/docs commit:
...
- Validation passed:
  ...
  - full deterministic ETF adapter unit file: `443 passed in 17.69s`
  - strict manifest recompute: `450` registered / `332` native /
    `118` fallback / gap `46`
  - opt-in live-provider manifest/accounting invariants:
    `2 passed, 338 deselected`
  - Ruff for changed backend files: passed
...
- Added a sixth named reconciliation batch from the StockAnalysis ETF provider
  table captured on `2026-07-28` (`469` named provider rows ranked by assets,
  ETF count, and average expense ratio).
- Registered `10` high-ranked distinct provider identities as explicit audited
  fallback-only adapters under `needs_first_party_route_discovery`:
  `putnam`, `columbia_threadneedle`, `mfs`, `bluemonte`, `vistashares`,
  `ershares`, `portfolio_building_block`, `kovitz`, `sapient`, and
  `strategas`.
- Added StockAnalysis alias dispositions for display names/product lines that
...
- Current strict matrix after this source-reconciliation batch: `430`
  registered / `332` native-live-backed / `98` fallback-only / target `496`
  promoters / `66` registered-promoter gap / `generated_recognition_only []`.
...
- Validation passed:
  ...
  - full deterministic ETF adapter unit file: `441 passed in 17.71s`
  - strict manifest recompute: `430` registered / `332` native /
    `98` fallback / gap `66`
  - opt-in live-provider manifest/accounting invariants:
    `2 passed, 338 deselected`
  - Ruff for changed backend files: passed
...
- Continued under the corrected broad-market task scope: the current target is
  the LSEG Lipper U.S. ETF promoter universe (`496` promoters as of
  `2026-06-30`).
- Reconciled a fourth named source batch from `https://etfdb.com/issuers/`,
  using the same ETFDB/VettaFi issuer league table extracted from a source that
  states its calculations are based on U.S.-listed ETFs and one issuer per ETF.
- Added `7` confirmed named identities as explicit audited fallback-only
...
- Current strict matrix after the batch: `419` registered / `326`
  native-live-backed / `93` fallback-only / target `496` promoters / `77`
  registered-promoter gap / `generated_recognition_only []`.
- This batch is named-provider reconciliation, not native route completion.
  Remaining ETFDB rows are now mostly spelling, legal-parent, or route-owner
  aliases of existing adapters and need careful mapping before any additional
  registration.
- Validation passed:
  - focused fourth-batch registry slice: `7 passed, 421 deselected`
  - full deterministic ETF adapter unit file: `428 passed in 17.40s`
  - opt-in live-provider manifest/accounting slice:
    `2 passed, 332 deselected`
  - Ruff for changed backend files: passed
  - `git diff --check`: passed
...
- Continue source-backed reconciliation for the remaining `77` identities.
  Before adding more ETFDB rows, resolve whether each remaining row is a true
  distinct promoter identity, a legal-parent alias, or already covered by a
  brand/platform route. Then continue native route discovery by ETF count, AUM,
  and U.S. market coverage.
...
- Verified the configured count remains `277/345` native/live-backed and
  `68/345` fallback-only. This is configuration coverage, not a newly green
  matrix result.
- Reproduced ETF Architect QVAL's issuer-edge defect: its official page is
  publicly indexed with a complete current holdings table, but backend-style
  retrieval currently returns an empty HTTP `200` body (and previous retries
  produced issuer-side `502`). No alternate stable issuer-owned artifact was
  found, so the adapter was not falsely changed or promoted.
- A fresh full opt-in live matrix was launched but this execution channel
  returned before pytest emitted a completion footer. It must be rerun in a
  durable runner before claiming current full-matrix health.

### Next step

- Continue the remaining issuer audit, starting with actual ETF publishers
  that expose an executable issuer-native complete holdings artifact. Preserve
  the exact provider/static/live test requirement for every promotion.

## Adapter invariant validation - 2026-07-21T19:35Z

- The full deterministic ETF adapter suite passed: `347 passed`.
- With `RUN_LIVE_ETF_HOLDINGS_TESTS=1`, both registry invariants passed. They
  prove every configured native/live-backed provider is represented by a
  concrete issuer-route live test and that the test manifest matches the
  registry exactly. This validates coverage wiring, not live endpoint health.

## Remaining priority-source audit - 2026-07-21T18:35Z

- WisdomTree's publicly indexed U.S. WTV page still advertises complete
  holdings, but the documented Fund Data API requires a private
  `x-wt-dataspan-key` and direct issuer-page retrieval remains WAF-blocked.
  Its indexed top-ten preview is not a complete portfolio and must not be
  parsed as one.
- SoFi's SFY page exposes top-ten positions and periodic filings only. No
  complete current issuer-native holdings artifact was found.
- The registered `marathon` identity refers to Marathon Asset Management as
  JHHY's subadvisor; John Hancock/Manulife is the actual ETF publisher. Do not
  create an issuer adapter for a subadvisor or count it as native provider
  support.
- Strict state remains `277/345` native/live-backed and `68/345` fallback-only.

### Next step

- Continue remaining issuer-owned routes only. Investigate Manulife/John
  Hancock as the actual publisher separately from the non-publisher Marathon
  identity; do not promote source-blocked providers without a concrete
  complete first-party artifact and matching live test.

## Remaining issuer source audit - 2026-07-21T17:31Z

- Rechecked priority and remaining issuer-native sources without promoting an
  adapter on search-index evidence alone. WisdomTree's official U.S. product
  page advertises a complete holdings action, but its public product and
  DataSpan endpoints remain Cloudflare/API-key gated to backend requests.
  Thrivent's official ETF pages advertise fund-scoped daily holdings downloads,
  but those downloads remain Imperva/CloudFront gated.
- Westwood's official MDST product page exposes a concrete complete holdings
  CSV (`1471-MDST/1471-Holdings.csv`), but the direct issuer download returns
  an interactive Cloudflare challenge to a browser-header backend request.
  The same source pattern covers WEEI and YLDW. No adapter was promoted,
  because none of these routes can currently satisfy the required live test.
- Strict state remains `276/345` native/live-backed integrations and `69/345`
  fallback-only. The 16-provider expansion is still uncommitted implementation
  work; the last committed checkpoint records `260/345`.

### Next step

- Continue the remaining issuer queue and only promote a provider after an
  executable complete issuer-native source, isolated implementation, static
  parsing test, and opt-in live route test all pass.

## Unlimited Funds native route - 2026-07-21T20:15Z

- Promoted `unlimited` through isolated `HFND`, `HFGM`, `HFMF`, and `HFEQ` routes.
  The adapter accepts only their verified issuer-owned, fund-scoped current CSVs,
  filters strictly to the requested account, and preserves cash/futures as
  non-tradable source rows.
- Unlimited's public WordPress download edge rejects `httpx` but serves the same
  official CSV to `requests`, so the compatibility fallback is constrained to
  this provider's verified routes. Focused deterministic coverage, the full
  adapter suite (`344 passed`), Ruff, exact opt-in `HFND` live route, provider
  matrix, and concrete-live-route invariant passed. Strict state: `276/345`
  native/live-backed and `69/345` fallback-only.

### Next step

- Continue the remaining issuer queue using only current, complete, first-party
  holdings artifacts. Do not count SEC, top-ten, stale reports, or third-party
  disclosure as native provider support.

## Lagan / Congress Asset Management native route - 2026-07-21T16:57Z

- Promoted `lagan` through isolated `CSMD`, `CAML`, and `CAFX` routes. The adapter
  validates the official Congress product page, its declared application script,
  and the exact shared FilePoint CSV before accepting only the requested account's
  rows. Cash and fixed-income positions remain source-faithful and non-tradable.
- Focused deterministic coverage, Ruff, the exact opt-in `CAML` live route, the
  provider-matrix invariant, and concrete-live-route invariant passed. Strict
  state is `275/345` native/live-backed and `70/345` fallback-only.

### Next step

- Continue the remaining issuer queue with only current, complete, issuer-native
  holdings sources. Do not count SEC, top-ten, stale reports, or third-party
  artifacts as primary provider support.

## ShariaPortfolio / SP Funds native route - 2026-07-21T19:40Z

- Promoted `shariaportfolio` through isolated `SPTE` and `SPWO` adapters. Each
  validates its official SP Funds product page declares its exact account-scoped
  daily CSV before ingesting only the requested fund's holdings. Cash and dated
  option contracts remain non-tradable source rows.
- Focused deterministic coverage, the full adapter suite (`342 passed`), Ruff,
  exact opt-in `SPTE` live route, the provider-matrix invariant, and the
  concrete-live-route invariant passed. Strict state is `274/345`
  native/live-backed and `71/345` fallback-only.

### Next step

- Continue the remaining issuer queue. Symmetry's official product surface and
  Q3's official product surface both returned WAF challenge pages before a
  current complete holdings artifact could be retrieved; retain both as
  fallback-only and investigate the next issuer-owned route.

## Gladius / Wayfinder native route - 2026-07-21T19:20Z

- Promoted `gladius` through an isolated `CMBO` adapter. It accepts only the
  verified Wayfinder product page, validates the fund identity, parses the
  issuer-rendered holdings dataset, and rejects it unless the disclosed weights
  total the complete portfolio. Options and cash stay non-tradable source rows.
- Focused deterministic coverage, the full adapter suite (`341 passed`), Ruff,
  exact opt-in `CMBO` live route, the provider-matrix invariant, and the
  concrete-live-route invariant passed. Strict state is `273/345`
  native/live-backed and `72/345` fallback-only.

### Next step

- Continue the remaining issuer queue. Investigate only current, complete,
  issuer-native routes; do not promote top-ten, stale-report, SEC, or
  third-party artifacts as primary provider support.

## Nightview Capital native route - 2026-07-21T19:05Z

- Promoted `nightview` through an isolated `NITE` adapter. It validates that the
  issuer's own fund page declares its exact complete daily CSV before parsing the
  current portfolio. The disclosure provides security descriptions rather than
  exchange symbols, so the adapter deliberately leaves symbols unset for the
  existing identifier/provider reconciliation layer rather than guessing them.
- Focused deterministic coverage, the full adapter suite (`340 passed`), Ruff,
  exact opt-in `NITE` live route, the provider-matrix invariant, and
  concrete-live-route invariant passed. Strict state is `272/345`
  native/live-backed and `73/345` fallback-only.

### Next step

- Continue the remaining issuer queue. Re-audit another issuer-owned complete
  portfolio route; do not substitute SEC, stale reports, top-ten data, or a
  generic feed for a native provider integration.

## Reckoner Capital Management native route - 2026-07-21T18:40Z

- Promoted `reckoner` through an isolated `RAAA` adapter. It validates the
  official fund page, its issuer portfolio iframe, the iframe's declared script,
  and that script's exact complete daily CSV. CLO debt and repo rows remain
  source-faithful non-tradable positions.
- Reckoner's public edge returns `403` to `httpx` but serves the same verified
  issuer URLs to `requests`; the adapter's transport fallback is strictly scoped
  to this issuer chain. Focused deterministic coverage, the full adapter suite
  (`339 passed`), Ruff, exact opt-in `RAAA` live route, provider-matrix invariant,
  and concrete-live-route invariant passed. Strict state is `271/345`
  native/live-backed and `74/345` fallback-only.
...
## Myriad Asset Management native route - 2026-07-21T18:25Z

- Promoted `myriad` through an isolated `MDAA` adapter. It validates the official
  fund page, its declared holdings application script, and that script's exact
  complete daily CSV before accepting only `MDAA` account rows. Currency futures
  and cash stay non-tradable rather than being represented as invented symbols.
- Focused deterministic coverage, the full adapter suite (`338 passed`), Ruff,
  the exact opt-in `MDAA` live route, provider-matrix invariant, and
  concrete-live-route invariant passed. Strict state is `270/345` native/live-backed
  and `75/345` fallback-only.

## SRN Advisors / Siren ETF Trust native route - 2026-07-21T18:05Z

- Promoted `srn` through an isolated Siren ETF Trust adapter for `BLCN` and
  `LEAD`. It validates the exact issuer product page declares the shared daily
  CSV before filtering only the requested `Account` rows. It retains valid
  identifiers but keeps cash/currency positions non-tradable.
- The live `BLCN` portfolio is currently in liquidation and truthfully contains
  only seven disclosed cash/currency rows. Deterministic parser coverage, the
  exact opt-in live BLCN route, the provider matrix, concrete-live-route
  invariant, Ruff, and the full deterministic suite (`337 passed`) passed.
  Strict state is `269/345` native/live-backed and `76/345` fallback-only.

### Next step

- Continue the remaining issuer queue. WisdomTree remains source-blocked by
  Cloudflare; investigate other unresolved first-party issuer routes rather than
  substituting SEC, stale reports, top-ten data, or generic feeds.

## Retireful / Mohr Funds native route - 2026-07-21T17:25Z

- Promoted `retireful` through an isolated Mohr Funds adapter for `RULE`, `SNAV`,
  `MFUL`, and `CNAV`. It validates the issuer page/fund identity, follows only
  issuer-declared pagination links, and requires all parsed rows to equal the
  page's declared holdings count. Cash-equivalent rows without ticker/CUSIP are
  retained as non-tradable cash rather than dropped.
- Deterministic pagination/cash parsing coverage, the exact opt-in live `RULE`
  route, the provider matrix, and the concrete-live-route invariant passed.
  Strict state is now `268/345` native/live-backed and `77/345` fallback-only.

## Guinness Atkinson and Westwood source retries - 2026-07-21T16:55Z

- Retried Guinness Atkinson's official `DIVS` product page and Westwood's
  official `MDST` product page with browser-equivalent retrieval. Both return
  Cloudflare challenge documents before their advertised complete holdings
  downloads can be traced to a current executable issuer artifact.
- Neither issuer is promoted or counted. Continue the remaining queue; do not
  substitute SEC, third-party, top-ten, or stale-report data for either native
  provider route.

## Beacon Capital product-line completion - 2026-07-21T16:40Z

- Extended the existing isolated Beacon adapter from `BSR` to both active Beacon
  ETFs: `BSR` (Unified Catalyst) and `BTR` (Tactical Risk). Each route validates
  its own official product page and page-declared complete current holdings CSV;
  metadata and source-row identifiers are now fund-specific rather than being
  stamped `BSR` for every result.
- Deterministic BSR/BTR fixtures, both exact opt-in live routes, the provider
  matrix, and the concrete-live-route invariant passed. This completes Beacon's
  active ETF product coverage but does not change strict issuer coverage:
  `267/345` native/live-backed and `78/345` fallback-only.

## PlanRock source audit - 2026-07-21T16:25Z

- The official PlanRock literature page is backend-readable and identifies PRAE,
  but its only complete holdings documents are dated `2025-06-30` and
  `2025-12-31` PDFs. It does not expose a current holdings table, CSV, API, or
  a matching complete PRMN portfolio artifact.
- Do not promote `planrock` through an old report, SEC, or third-party data.
  It remains fallback-only until a current, executable issuer-native route is
  published; continue the remaining provider queue.

## Beacon Capital native route - 2026-07-21T16:15Z

- Promoted `beacon_capital` through an isolated `BSR` adapter. It validates the
  issuer's official Unified Catalyst page and only its page-declared complete
  holdings CSV. Cash sweep and receivable rows remain non-tradable, while normal
  portfolio ETF tickers retain their CUSIPs and weights.
- Beacon's WordPress edge rejects httpx for both public documents but accepts
  requests; that fallback is constrained to these two exact verified BSR URLs.
  Focused static coverage, the full deterministic suite (`334 passed`), exact
  opt-in live BSR route, provider matrix, concrete-live-route invariant, Ruff,
  and `git diff --check` passed. Strict state is now `267/345` native/live-backed
  and `78/345` fallback-only.

## SWP Investment Management native route - 2026-07-21T16:00Z

- Promoted `swp` through an isolated `SWP` adapter. It first validates that the
  issuer's public fund page declares the exact daily holdings spreadsheet, then
  parses only the `SWP` account rows from that file. Standard equity tickers are
  exposed as tradable symbols while option and cash rows are retained as
  non-tradable source data.
- Focused static coverage, the full deterministic adapter suite (`333 passed`),
  the exact opt-in live `SWP` route, provider matrix, concrete-live-route
  invariant, Ruff, and `git diff --check` passed. Strict state is now `266/345`
  native/live-backed and `79/345` fallback-only.

## WisdomTree source retry - 2026-07-21T15:40Z

- Retried the official U.S. `WTV` product page after search indexing confirmed
  that it advertises a full-holdings feature. Direct issuer retrieval still
  returns `403` before the complete artifact can be traced or tested.
- `wisdomtree` remains fallback-only and uncounted. No top-ten page data, SEC
  filing, or third-party substitute was promoted as a native route.

## Waverly Advisors native route - 2026-07-21T15:30Z

- Promoted `waverly` through an isolated `GGM` adapter. It validates Waverly's
  official GGM product page and its page-declared complete holdings CSV before
  parsing only that portfolio. The adapter normalizes bare issuer percentage
  points and preserves cash/receivables as non-tradable rows.
- Static parsing coverage, the full deterministic adapter suite (`332 passed`),
  exact opt-in live GGM coverage, the provider matrix, and the
  concrete-live-route invariant passed. Strict state is now `265/345`
  native/live-backed and `80/345` fallback-only.

## SoFi source retry - 2026-07-21T15:05Z

- Retried the official `SFY` product page with browser-equivalent direct
  retrieval. The issuer returned a `403` challenge response before exposing a
  product-page document route. Current issuer search indexing confirms top-ten
  holdings and quarterly reports, but neither is an executable complete current
  portfolio source for a native adapter.
- `sofi` remains fallback-only and uncounted. No SEC, administrator, or
  third-party source was substituted; continue source discovery after the next
  reachable issuer candidate.

## WEBS Investments native route - 2026-07-21T14:50Z

- Promoted `webs` through an isolated issuer API adapter. It first verifies the
  requested ETF against WEBS Investments' public catalogue and matching fund
  metadata, then reads only that ticker's daily metrics and complete holdings
  route. Cash and total-return-swap rows remain non-tradable rather than being
  represented as fabricated instruments.
- The actual native adapter returned four current `DVSP` rows. Its static
  fixture, exact opt-in live route, registered-provider matrix,
  concrete-live-route invariant, full deterministic adapter suite (`331
  passed`), Ruff, and `git diff --check` passed. Strict state is now
  `264/345` native/live-backed and `81/345` fallback-only.

## X-Square native route - 2026-07-21T14:35Z

- Promoted `x_square` through an isolated `ZTAX` adapter. It validates the
  exact X-Square Municipal Income ETF complete-holdings page, uses only the
  token/API configuration declared by that page, and confirms `fundsymbol` and
  `fullpartial` before accepting rows. Municipal-debt holdings retain their
  identifiers and valuation while remaining non-tradable fixed-income rows.
- The actual native adapter returned 16 current ZTAX holdings. Its static
  fixture, exact opt-in live route, registered-provider matrix,
  concrete-live-route invariant, full deterministic adapter suite (`330
  passed`), Ruff, and `git diff --check` passed. Strict state is now
  `263/345` native/live-backed and `82/345` fallback-only.

## Donoghue Forlines native route - 2026-07-21T14:12Z

- Promoted `donoghue_forlines` through its existing isolated `DFTT` adapter.
  The current official Tactical 30 page and its declared nonce-bearing
  fund-scoped CSV are executable. The adapter now uses a bounded
  browser-compatible `requests` fallback only when the issuer rejects httpx
  with 403, then validates the same page-declared CSV route.
- The actual native adapter returned 33 current DFTT holdings. Its focused
  static tests, exact opt-in live route, registered-provider matrix,
  concrete-live-route invariant, full deterministic adapter suite (`329
  passed`), Ruff, and `git diff --check` passed. Strict state is now
  `262/345` native/live-backed and `83/345` fallback-only.

## Artemis native route - 2026-07-21T13:57Z

- Promoted `artemis` through an isolated ARS Investment Partners adapter for
  `ACEP` and `AFOS`. It verifies the exact official product page and fund
  identity before parsing only the embedded complete holdings table, retaining
  ticker, CUSIP, shares, weight, market value, and effective date. Cash rows
  remain non-tradable.
- Static parser coverage, exact opt-in `ACEP` live coverage, the registered
  provider matrix, concrete-live-route invariant, Ruff, and `git diff --check`
  passed. Strict state is now `261/345` native/live-backed and `84/345`
  fallback-only.
...
- WisdomTree remains a high-value unresolved issuer, but its official U.S.
  product page for `WTV` returned a Cloudflare block to a browser-equivalent
  request. Search-indexed pages confirm that the issuer renders holdings and a
  `View all holdings` control, but an indexed top-ten view is not a complete,
  executable source for a native adapter. The existing placeholder adapter must
  remain uncounted until the product page's actual complete response can be
  retrieved and live-tested.
- BMO's public holdings tool was also checked with `FEBT`, a U.S.-listed ETF.
  The page exposes a cached `etfs/holdings` response containing an empty
  holdings array and identifies the fund as AllianzIM, not a BMO-managed
  portfolio. This market-wide tool is not a BMO issuer-native complete holdings
  source. BMO remains fallback-only; do not promote it through this route.
- Strict state is unchanged: `345` registered, `256` native/live-backed, and
  `89` fallback-only. Continue with a different remaining provider that has an
  executable first-party full-portfolio artifact.

## Precidian source audit - 2026-07-21T14:15Z

- Precidian is the ActiveShares ETF structure provider rather than the public
  portfolio publisher for the funds using that structure. Its own public
  material states that ActiveShares protects portfolio confidentiality and does
  not disclose holdings daily. The available first-party disclosure cadence is
  therefore not sufficient for the required native, current holdings
  integration.
- Do not count `precidian` as native/live-backed or add a thin SEC-backed
  implementation. Its constituent detail can only be obtained through the
  actual fund publisher's independent route when one exists, or through the
  existing SEC filing fallback.
- The unchanged current adapter suite passed: `323 passed`.

## Guardian Capital source audit - 2026-07-21T14:30Z

- Guardian's U.S.-listed `LCG` identity is represented through its Sterling
  Capital affiliate. The official Sterling LCG product page is reachable and
  confirms the fund identity, but it does not render a complete holdings table
  despite retaining client-side table initialization code.
- Its only disclosed quarterly holdings links currently return `404` from the
  official FilePoint host. A stale linked document is not a valid native source.
  Do not promote `guardian` until the affiliate publishes an executable complete
  report or table; the separate live-backed `sterling_capital` adapter does not
  make the Guardian registry identity natively supported.

## Current adapter integrity validation - 2026-07-21T13:35Z

- The current uncommitted adapter implementation remains deterministic-test
  clean: `323 passed` for `tests/unit/services/test_etf_holdings_adapters.py`.
- Both live manifest gates passed with live testing enabled: the registry gate
  verifies `live_tested_default_route` exactly matches the native set, and the
  concrete-route gate verifies every one of the `256` native/live-backed
  providers has a declared isolated live route. These gates validate the
  supported set's maintainability; they do not make the remaining `89`
  fallback-only identities complete.

- A fresh full opt-in live matrix was started immediately afterwards, but this
  execution channel terminated before producing its pytest footer after it had
  begun the provider cases. Do not represent that invocation as pass or fail.
  The next worker should rerun the full matrix through the normal durable test
  runner, then continue the remaining provider queue.

## Pacific Investment Management Company / PIMCO source audit - 2026-07-21T13:25Z

- Revalidated PIMCO's official `BOND` product page and decoded its public
  `FundIntegration` configuration. The page identifies CUSIP `72201R775` and
  the issuer-owned `https://fund-ui.pimco.com/fund-detail-api` application.
- The public application exposes only top-ten holdings methods. Its documented
  `/api/as-of-date`, `/api/funds/{cusip}`, and top-ten export calls require an
  authenticated role: requests with the public page's browser headers return
  `401`, while supplied role variants return issuer-side `500` errors. Carrying
  only cookies issued by PIMCO's public product page does not change that
  result.
- This is not an executable, complete first-party holdings route. Do not add a
  PIMCO adapter or count `pacific_investments` until a public full-portfolio
  artifact can be verified. SEC remains fallback-only. The next queue attempt
  should use a different remaining issuer rather than retrying this same
  authenticated API path.

## Manulife / John Hancock source audit - 2026-07-21T13:30Z

- Rechecked the issuer's official U.S. ETF catalogue for a complete holdings
  route. The public catalogue endpoint returns an Akamai `Access Denied` page
  to browser-equivalent backend retrieval before a fund-specific portfolio can
  be discovered.
- Publicly indexed material confirms product identities and pricing but provides
  only factsheets/top-ten descriptions. It is insufficient for a complete
  provider-native constituent integration. `manulife` remains fallback-only;
  do not substitute SEC or third-party holdings pages as native support.

## Concourse / ETF Architect source audit - 2026-07-21T12:55Z

- Verified that Concourse's active US ETF is `CCFE` and that its administrator,
  ETF Architect, publicly lists a corresponding holdings download in its fund
  directory. The official Concourse product site and the ETF Architect funds
  directory are both currently Cloudflare-blocked to the browser-equivalent
  backend request used by adapters.
- The blocked directory does not expose the concrete downloadable artifact to
  this runtime, so no bounded native adapter can yet validate the requested ETF
  and complete portfolio. Concourse remains fallback-only; do not substitute
  SEC or a third-party holdings page for the missing executable primary route.

## Absolute Investment Advisers source audit - 2026-07-21T11:50Z

- Verified the official ABEQ product page and its issuer-linked FilePoint
  workspace. The workspace identifies the current fund (`fundID=1250`) and
  declares both a CSV download and the scoped `absolute_getholdings_cached2.php`
  API route.
- Neither route currently yields a usable complete portfolio to a
  browser-equivalent backend request: the declared CSV responds with an opaque
  35-byte payload, while the declared JSON API responds `Name is empty`.
  Absolute therefore remains fallback-only and uncounted; do not replace this
  failed native route with SEC fallback or a third-party holdings source.

## Full native-provider matrix revalidated - 2026-07-21T12:10Z

- After issuer-local retries were hardened for Cultivar, ETF Architect, and
  Toews, the complete opt-in live provider matrix passed: `263 passed in
  369.84s`. The deterministic adapter suite also passed (`323 passed`), and the
  registry/concrete-live-route manifest invariants passed.
- The verified strict state is `256/345` native/live-backed providers and
  `89/345` fallback-only. Continue the source-audited provider queue; do not
  treat the clean matrix as completion of the all-345 objective.

## Grace Partners / First Trust native route - 2026-07-21T11:06Z

- Promoted `grace_partners` through its own bounded adapter for the verified
  First Trust publisher relationship. It accepts only the exact public First
  Trust holdings page for the requested ETF, validates the page's ticker/title,
  and parses the complete holdings table with identifiers, CUSIPs, quantities,
  values, and weights.
- Validation passed: focused static adapter coverage; opt-in live `IDVY` route
  (`1 passed`); and Ruff. The live route was rerun with external network access
  after the sandbox-only attempt failed DNS resolution, so the route is verified
  rather than inferred. Strict coverage is now `256/345` native/live-backed and
  `89/345` fallback-only. The full 263-case matrix has not yet been rerun after
  the Pettee and Grace additions.

## Priority-source retry - 2026-07-21T11:15Z

- Retried the remaining high-impact issuers from the agreed priority order with
  browser-equivalent backend requests. All documented WisdomTree US product URL
  variants return the same Cloudflare `403`; SoFi's public SFY page and its
  advertised symbol-scoped holdings endpoint both return `403`; and both public
  and financial-professional Thrivent ETF product pages return `403`.
- These are source-access blockers, not a reason to route those issuers through
  SEC as if it were a native integration. They remain fallback-only while the
  provider queue moves to other executable issuers.

## Live-route reliability repairs - 2026-07-21T11:35Z

- A fresh 263-case live sweep reached `261 passed` but exposed three transient
  issuer-side failures: Cultivar `CVAR` timed out, ETF Architect `QVAL` returned
  a `502` through its issuer-compatible requests transport, and Toews `HRSK`
  exhausted two timeout retries.
- Cultivar and Toews now use bounded three-attempt timeout retries; ETF Architect
  retries transient `429/5xx` results from its narrowly scoped issuer transport.
  Each behavior has deterministic regression coverage and the focused live
  `CVAR`, `QVAL`, and `HRSK` routes pass. Rerun the complete matrix before
  claiming a new all-green full sweep.

## Pettee/Hoya native route - 2026-07-21T12:45Z

- Promoted `pettee` through an isolated Hoya Capital implementation bounded to
  `HOMZ` and `RIET`. It verifies the matching Hoya product page before accepting
  only its declared `download-holdings-usbanks.php` complete `.xls` portfolio,
  preserving ticker, CUSIP, shares, value, and weight while retaining cash as
  non-tradable.
- Validation passed: deterministic adapter suite (`319 passed`); exact opt-in
  live `HOMZ` route; registry and concrete-live-route manifest invariants;
  Ruff; and `git diff --check`. Strict coverage is now `255/345`
  native/live-backed and `90/345` fallback-only.

## Remaining-provider source audit - 2026-07-21T12:25Z

- Rechecked two remaining registered identities before attempting another native
  adapter. SoFi's official ETF pages advertise a symbol-scoped holdings API, but
  that API and all product pages return the same Cloudflare `403` challenge to
  browser-equivalent backend requests. It remains fallback-only rather than
  being promoted on the basis of a non-executable endpoint.
- The apparent CI Financial/FundX lead is not a usable issuer route: the public
  FundX domain is an investment-advisory site and does not publish the ETF
  holdings artifacts for the former FundX ETF products. No adapter was added and
  strict coverage remains `254/345` native/live-backed and `91/345` fallback-only.

## Live-provider reliability validation complete - 2026-07-21T12:10Z

- The complete opt-in live-provider matrix is now clean: `261 passed in
  421.40s`. The full deterministic adapter suite also passed (`318 passed`),
  as did Ruff and `git diff --check`.
- The native count is therefore verified, not provisional: `254/345`
  provider identities have a bounded, first-party, concrete executable route;
  `91/345` remain fallback-only and do not count as native support.
- Reliability repairs covered Morgan Stanley's changed public download path,
  Regan timeout retry, Baron stale dated-CSV recovery, ETF Architect timeout
  retry, and Howard Capital rate-limit retry. Resume the remaining issuer queue
  without demoting or falsely promoting providers.

## Live-provider reliability repair in progress - 2026-07-21T11:30Z

- A complete opt-in `261`-case live-provider sweep exposed Morgan Stanley `MSLC`:
  its product page changed the public workbook path from `/pub/wealth-...` to
  `/wealth-...`. The bounded native matcher now accepts both official forms and
  has a deterministic regression test; its targeted live route passes.
- The following full sweep exposed Regan `MBSF` timing out under cumulative
  load. The isolated Regan adapter now retries only transient timeouts three
  times with bounded backoff and has a timeout-then-success regression test;
  its targeted live route passes.
- A new complete live sweep is in progress. Do not claim `254/345` as a fully
  verified live-executable count until it completes cleanly. It completed with
  `258 passed, 3 failed in 345.08s`: Baron `RONB` resolved a stale dated CSV
  path (`404`), ETF Architect `QVAL` timed out, and Howard Capital `QQH`
  received `429`. Repair or demote those exact routes before continuing the
  remaining `91` provider queue.

## Sovereign's Capital native route - 2026-07-21T10:00Z

- Promoted `sovereign` through an isolated, strictly bounded `SOVF` adapter. It
  verifies Sovereign's Capital's public SOVF page before following only its exact
  declared complete XLS workbook, preserving ticker, CUSIP, shares, market value,
  weight, and cash semantics.
- Validation passed: Ruff; complete deterministic adapter suite (`316 passed`);
  exact opt-in `SOVF` live route; and both configured-provider/concrete-live-route
  invariants (`3 passed`). Strict state is now `254/345` native/live-backed and
  `91/345` fallback-only. SEC EDGAR remains a fallback only and does not count
  toward native provider coverage.
- Continue the remaining issuer queue with another backend-reachable, complete,
  first-party artifact; do not promote search-indexed, stale, or partial sources.

## Remaining-provider access audit - 2026-07-20T19:45Z

- Retried high-priority remaining providers with their actual first-party routes.
  Guinness Atkinson's official resources catalogue exposes complete ETF schedules
  of investments, but direct backend retrieval of the linked PDF is currently
  blocked by a Cloudflare challenge (`403`). DoubleLine's official ETF catalogue
  is likewise Cloudflare-blocked (`403`). Q3 Asset Management's official QVOY
  page currently returns a Cloudflare `503` with a one-hour retry window.
- STF is now visibly published through Shelton Capital Management, but its official
  TUG product page's advertised live `tug-holdings` link currently resolves to
  `404`; its remaining official documents are periodic PDFs, not a verified current
  holdings endpoint. It is not promoted until a working current, complete issuer
  artifact is available.
- Strict coverage remains `253/345` native/live-backed and `92/345` fallback-only.
  Continue with another reachable remaining issuer artifact. SEC EDGAR remains
  fallback-only and does not count toward native provider coverage.

## Sound Capital native route - 2026-07-20T19:00Z

- Promoted `sound_capital` through an isolated, bounded `RVER` route. The adapter
  verifies River1's official Trenchless Fund ETF product page and follows only its
  exact declared full-holdings XLS workbook. River1 is recorded as the publisher;
  cash and other-assets rows stay non-tradable rather than becoming platform symbols.
- Validation passed: Ruff; complete deterministic adapter suite (`315 passed`);
  exact opt-in `RVER` live route; and both registry invariants (`3 passed`). Strict
  state is now `253/345` native/live-backed and `92/345` fallback-only.
- Continue with another reachable remaining issuer artifact. SEC EDGAR remains a
  fallback only and does not count toward native provider coverage.

## Little Harbor native route - 2026-07-20T18:43Z

- Promoted `little_harbor` through isolated, bounded `MSTB`, `MSTQ`, and `RMIF` routes.
  It verifies each Little Harbor public product page and follows only its declared
  full-holdings legacy-XLS download. Cash, futures/options, and Treasury holdings remain
  non-tradable rather than being treated as platform equity symbols.
- Validation passed: Ruff; complete deterministic adapter suite (`314 passed`);
  exact opt-in `MSTB` live route; and both registry invariants (`3 passed`). Strict
  state is now `252/345` native/live-backed and `93/345` fallback-only.
- Continue with another reachable remaining issuer artifact. WisdomTree and SoFi remain
  documented access blockers, not native support.

## Langar native route - 2026-07-20T18:24Z

- Promoted `langar` through an isolated, bounded `LGHT` route. The adapter validates
  Langar's public fund-settings identity before reading only the matching Nottingham
  publisher fund-scoped holdings JSON. It allows only harmless issuer spelling
  variation (`Health Tech`/`HealthTech`) while still enforcing the fund number and ticker.
- Validation passed: Ruff; complete deterministic adapter suite (`313 passed`);
  exact opt-in `LGHT` live route; and both registry invariants (`4 passed`). Strict
  state is now `251/345` native/live-backed and `94/345` fallback-only.
- WisdomTree and SoFi were retried against their official public product/holdings
  surfaces with browser-equivalent requests; all tested endpoints remained WAF/Cloudflare
  blocked and neither was promoted. Continue with another reachable issuer artifact.

## McIvy native route - 2026-07-20T18:12Z

- Promoted `mcivy` through an isolated, bounded Genter Capital route for `GEND`.
  The adapter validates the issuer's public fund-settings identity before reading
  only the matching Nottingham publisher fund-scoped holdings JSON. It retains
  CUSIP, SEDOL, shares, values, weights, and cash semantics without treating the
  publisher endpoint as a generic URL resolver.
- Validation passed: Ruff; complete deterministic adapter suite (`312 passed`);
  exact opt-in `GEND` live route; configured-provider registry invariant; and the
  concrete-live-route manifest invariant (`3 passed`). Strict state is now
  `250/345` native/live-backed and `95/345` fallback-only.
- Continue the remaining queue only with reachable first-party complete holdings
  artifacts. SEC EDGAR remains fallback-only and is not a counted native route.

## Mitsubishi UFJ native route - 2026-07-20T16:52Z

- Promoted `mitsubishi_ufj` through an isolated, bounded `MJSC` adapter. It
  fetches only MUFG's official product page and parses only its named complete
  Nuxt holdings component. Japanese venue tickers are retained as source
  metadata rather than being misrepresented as platform-tradable symbols.
- Validation passed: Ruff; complete deterministic adapter suite (`311 passed`);
  exact opt-in `MJSC` live route; configured-provider registry invariant; and
  the concrete-live-route manifest invariant (`3 passed`). Strict state is now
  `249/345` native/live-backed and `96/345` fallback-only.
- Continue the remaining queue only with reachable first-party complete holdings
  artifacts. SEC EDGAR remains fallback-only and is not a counted native route.

## BCP CC legacy-identity native route - 2026-07-20T16:38Z

- Promoted `bcp_cc` through its verified current publisher relationship with First
  Eagle. The adapter has its own bounded `FEGE`/`FEOE` route map and fetch flow;
  it uses only the corresponding public First Eagle ETF product pages rather than
  an arbitrary URL or SEC fallback. The relationship and publisher remain explicit
  in provenance metadata because BCP CC is a legacy issuer identity.
- Validation passed: Ruff; complete deterministic adapter suite (`310 passed`);
  exact opt-in `FEOE` live route; configured-provider registry invariant; and the
  concrete-live-route manifest invariant (`3 passed`). Strict state is now
  `248/345` native/live-backed and `97/345` fallback-only.
- Westwood and Anfield were also retried through their official pages, but each
  returned a browser-verification/`403` response before a complete portfolio could
  be retrieved. Neither was promoted. Continue the remaining queue only where a
  first-party, backend-retrievable complete holdings route can be verified.

## Remaining-provider access audit - 2026-07-20T16:30Z

- Rechecked three additional fallback-only issuers through their official public
  routes. Guinness Atkinson's resources catalogue remains Cloudflare-challenged
  (`HTTP 403`) even in a real headless browser. Donoghue Forlines' current DFTT
  product site returns the same class of Cloudflare `403` before a complete
  portfolio file is available. Manulife's U.S. John Hancock ETF surface returns
  Akamai `403` to both browser-header HTTP requests and a real headless browser.
- None is promoted: a search-indexed document or SEC filing is not an executable
  issuer-native holdings route. The strict support state remains `247/345`
  native/live-backed and `98/345` fallback-only.
- Exact next step: continue the remaining queue with a provider whose complete
  issuer artifact is reachable; add a bounded adapter, deterministic fixture,
  and opt-in concrete live test before increasing the count.

## Eighth Wonder native route - 2026-07-20T16:20Z

- Added an isolated Fundsmith `ETFT` adapter for Eighth Wonder. It validates the
  exact public ETF page and parses only its complete issuer-published holdings
  component, retaining global identifiers while surfacing platform symbols only
  where the issuer gives an applicable U.S. ticker. Cash remains non-tradable.
- Validation passed: Ruff; full deterministic adapter suite (`309 passed`);
  exact opt-in ETFT live route; configured-provider registry invariant; and
  concrete-live-route manifest invariant (`3 passed`). `anfield` was correctly
  removed from the counted set because its asserted source is not executable.
  The net strict count remains `247/345` native/live-backed and `98/345`
  fallback-only.

## WealthTrust native route - 2026-07-20

- Promoted `wealthtrust` through an isolated `WLTG` adapter. It verifies WealthTrust's
  official fund identity and parses only the complete public holdings table declared on
  that page, retaining FIGI, shares, market value, and decimal weight data while keeping
  cash non-tradable.
- Validation passed: Ruff; complete deterministic adapter suite (`307 passed`); exact
  opt-in WLTG live route; configured-provider registry invariant; and concrete-live-route
  manifest invariant (`4 passed`). Strict state is now `247/345` native/live-backed and
  `98/345` fallback-only. This is a verified provider increment, not completion of the
  remaining queue.
- Exact next step: continue the remaining-98 queue with the same isolated-adapter,
  first-party-route, static-fixture, and opt-in-live-test standard. SEC remains fallback only.

## Soundwatch native route - 2026-07-20T14:03Z

- Promoted `soundwatch` through an isolated `SHDG` adapter. It starts at Soundwatch's
  exact issuer product page, verifies the fund identity, and follows only the matching
  issuer-linked legacy-XLS workbook. The parser retains CUSIP, shares, market value,
  weights, and as-of date while keeping cash and index-option rows non-tradable.
- Validation passed: Ruff; complete deterministic adapter suite (`306 passed`); exact
  opt-in SHDG live route; configured-provider registry invariant; and concrete-live-route
  manifest invariant (`3 passed`). Strict state is now `246/345` native/live-backed and
  `99/345` fallback-only. This is a single verified provider increment, not a claim that
  the remaining queue is complete.
- Exact next step: continue the remaining-99 queue with the same isolated-adapter,
  static-fixture, and concrete-live-route threshold. SEC EDGAR remains fallback-only.

## Cultivar native route recovery - 2026-07-20T17:20Z

- Revalidated Cultivar's existing isolated CVAR fund-page adapter against the current
  issuer page, which returned `89` complete holdings dated `2026-07-17`. The former
  backend block no longer occurs, so the provider is now correctly promoted and has a
  concrete opt-in live route test.
- Validation passed: complete deterministic adapter suite (`305 passed`), exact CVAR
  live route, configured-provider registry invariant, and concrete-live-route manifest
  invariant (`4 passed`). Strict state is `245/345` native/live-backed and `100/345`
  fallback-only.
- Exact next step: continue the remaining-100 queue with issuer-native routes only;
  do not count an inaccessible parser or an SEC fallback as support.

## Marygold native route - 2026-07-20T17:00Z

- Promoted `marygold` through an isolated USCF Investments adapter limited to its
  verified U.S. ETF range. It starts at the matching USCF product page, requests the
  issuer's short-lived public browser token per refresh, and queries only the matching
  complete `holding/{symbol}/full` endpoint. The bearer token is never persisted.
- The adapter validates the returned fund symbol and complete-portfolio marker. Futures,
  cash, and other non-securities remain non-tradable instead of creating invented chart
  symbols. Validation passed: the complete deterministic adapter suite (`305 passed`),
  the exact opt-in USO live route, configured-provider registry invariant, and
  concrete-live-route manifest invariant (`3 passed`). Strict state is `244/345`
  native/live-backed and `101/345` fallback-only.
- Exact next step: continue the remaining-101 queue with the same isolated adapter,
  fixture, and concrete live-route threshold. Do not count source audits or SEC fallback.

## Remaining-provider source audit - 2026-07-20T16:30Z

- Revalidated three remaining fallback-only providers without changing the support
  count. Absolute Investment Advisers' currently discoverable ABEQ surface is
  access-gated or limited to strategy/investor documents; no complete current
  holdings artifact could be retrieved. SoFi's official ETF pages are indexed but
  backend requests remain WAF-blocked and only top-ten data is publicly exposed.
  BMO's reachable official endpoint remains Canadian/top-holdings-only rather than a
  complete U.S.-listed ETF portfolio feed.
- None meets the native-support threshold, so strict state remains `243/345`
  native/live-backed and `102/345` fallback-only. Do not convert these audits into
  implementation or SEC-fallback claims.
- Exact next step: continue the remaining queue with a provider that has a complete,
  backend-retrievable first-party artifact; add its bounded adapter, fixture, and
  opt-in live route before changing the count.

## AmeriLife native route - 2026-07-20T16:00Z

- Promoted `amerilife` through an isolated BAMA adapter. It verifies the exact
  Brookstone Asset Management product page and follows only its linked complete
  CSV portfolio. Publisher-format header whitespace is normalized before schema
  validation; CUSIP, shares, values, weights, and non-tradable sweep/receivable
  semantics are preserved.
- Validation passed: Ruff; full adapter unit suite (`304 passed`); exact opt-in BAMA
  and APMU live routes; configured-provider registry invariant; and concrete-live-route
  manifest invariant (`4 passed`). Strict state is now `243/345` native/live-backed and
  `102/345` fallback-only. This does not claim a fresh complete live matrix.
- Exact next step: continue the remaining-102 queue with another backend-reachable,
  complete first-party artifact. Do not count a provider without an isolated adapter,
  fixture, and concrete opt-in live-route test; SEC remains fallback-only.

## Envestnet native route - 2026-07-20T15:15Z

- Promoted `envestnet` through an isolated ActivePassive adapter. It starts from the
  exact official product page for the requested ticker, verifies the issuer-declared
  page identity, and follows only its matching full-holdings XLS download. The legacy
  workbook parser preserves identifier, shares, market-value, and weight data; the
  issuer page's disclosed as-of date is retained in snapshot metadata.
- Validation passed: Ruff; full adapter unit suite (`303 passed`); exact opt-in APMU
  live route; configured-provider registry invariant; and concrete-live-route manifest
  invariant (`3 passed`). Strict state is now `242/345` native/live-backed and
  `103/345` fallback-only. This does not claim a fresh complete live matrix.
- Exact next step: continue the remaining-103 queue with another backend-reachable,
  complete first-party artifact. Do not count a provider without an isolated adapter,
  fixture, and concrete opt-in live-route test; SEC remains fallback-only.
- Source audit immediately after this route: `absolute_investment_advisers` remains
  fallback-only. Its currently discoverable ABEQ product surface is access-gated or
  exposes only strategy and investor-document material; no backend-retrievable,
  complete current first-party holdings artifact has been verified. Do not promote it
  without an executable live route.

## Pictet native route - 2026-07-20T13:10Z

- Promoted `pictet` through an isolated PQUS adapter. It validates Pictet's public
  product page against the requested ticker and then fetches only its public Kurtosys
  complete fund-allocation API payload. It rejects mismatched fund identity, preserves
  ticker/CUSIP/ISIN/SEDOL/shares/value/weight/as-of data, and marks cash, derivatives,
  and fixed-income rows non-tradable where appropriate.
- Validation passed: Ruff; full adapter unit suite (`299 passed`); exact opt-in PQUS
  live route; configured-provider registry invariant; and concrete-live-route manifest
  invariant (`3 passed`). Strict state is now `238/345` native/live-backed and
  `107/345` fallback-only. The three agreed priority blockers (WisdomTree, SoFi, and
  Thrivent) were retried on 2026-07-20 and still return issuer bot-protection `403`s;
  they remain uncounted.
- Exact next step: continue the remaining-107 queue with another accessible complete
  first-party artifact. Do not count a provider without an isolated adapter, fixture,
  and opt-in live-route test; SEC remains fallback-only.

## Remaining-provider source audit - 2026-07-20T12:20Z

- Rechecked a reachable-looking remaining issuer rather than promoting it from search
  results: Guinness Atkinson publicly indexes its complete quarterly ETF holdings
  documents for ADIV, DIVS, MOTO, SOLR, GAID, GAUD, and GARA, but direct retrieval
  of the issuer resource catalogue from the backend receives a Cloudflare challenge
  (`HTTP 403`). No concrete issuer-native route can therefore be live-tested from
  this environment, so `guinness_atkinson` remains fallback-only and uncounted.
- Reconfirmed that the registered `guardian` and `bmo` identities currently resolve
  to Canadian ETF businesses, not an independently supported U.S.-listed ETF
  holdings route. John Hancock/Manulife documents advertise complete daily ETF
  holdings, but the resources are professional-login restricted and the issuer
  product surfaces reject backend requests with `HTTP 403`.
- Strict state is unchanged: `237/345` native/live-backed, `108/345` fallback-only.
  Exact next step: continue the remaining queue, prioritizing a complete current
  first-party artifact that is actually retrievable from the backend; add the bounded
  adapter, fixture, and opt-in live test before changing the count.
...
- Reconciled the apparent checkpoint-only history with the real working tree: the current
  uncommitted implementation batch is substantive and includes isolated, tested adapters
  for Praxis, Baird, Affiliated Managers Group, Raymond James, Osprey, and Redwood.
  The strict registry count is `230/345` native/live-backed and `115/345` fallback-only.
- Revalidated the complete local adapter suite (`290 passed`) and Ruff. The opt-in live
  checks for Redwood plus the registry/configuration and concrete-live-route invariants
  also passed (`3 passed`). This is not evidence that the full 237-case live matrix has
  completed; it has not yet had a final uninterrupted run.
- Re-audited two high-priority remaining sources instead of promoting them from search
  snippets: WisdomTree's actual U.S. product page continues to return a Cloudflare
  challenge to browser-header backend requests; Pictet's public fund pages render only
  a client-side Kurtosys application and do not expose a stable, directly fetchable
  complete holdings artifact in the issuer HTML. Both remain fallback-only and uncounted.
- Exact next step: continue source discovery from the remaining 115 keys, select only
  an issuer with an executable full portfolio route, then add a constrained adapter,
  fixture, opt-in live route test, and registry entry before incrementing the count.

## Redwood native holdings route - 2026-07-16T19:45Z

- Promoted `redwood` through an isolated adapter restricted to its verified LeaderShares
  ETF range: `LSAF`, `LSAT`, and `DYLD`. Each issuer product page links to a
  fund-scoped complete holdings CSV. The adapter validates the exact reported fund name,
  one as-of date, and the actual CSV schema; it preserves CUSIP, normalized ticker,
  shares, market value, weights, and cash/fixed-income semantics without inventing
  tradable symbols.
- Validation passed: Ruff; full adapter suite (`290 passed`); exact opt-in live `LSAF`
  route; configured-provider invariant; and concrete-live-route manifest invariant
  (`3 passed`). Strict state is now `230/345` native/live-backed and `115/345`
  fallback-only. The full live-provider matrix still has no completed final result.

## Osprey native holdings route - 2026-07-16T19:20Z

- Promoted `osprey` through an isolated adapter restricted to the verified public
  `OSOL`, `OBNB`, `ODOT`, and `OBNK` trust pages. Each page discloses daily asset
  units per trust share, AUM, outstanding trust shares, and an as-of date. The adapter
  derives the aggregate single-asset holding directly from those issuer values and keeps
  it as a crypto security rather than fabricating an equity constituent list.
- Validation passed: Ruff; full adapter suite (`288 passed`); exact opt-in live `OSOL`
  route; configured-provider invariant; and concrete-live-route manifest invariant
  (`3 passed`). Strict state is now `229/345` native/live-backed and `116/345`
  fallback-only. The full live-provider matrix still has no completed final result.

## Raymond James native holdings route - 2026-07-16T19:00Z

- Promoted `raymond_james` through an isolated adapter restricted to `RJDI`, `RJMI`,
  and `RJVI`. Raymond James's public FilePoint portal exposes each ETF's complete,
  fund-specific quarterly Schedule of Investments PDF at a stable issuer-owned route.
  The adapter validates the requested fund title and as-of date, preserves reported
  shares/par amounts and market values, and correctly distinguishes equity, fixed-income,
  and cash-equivalent positions without inventing symbols.
- Validation passed: Ruff; full adapter suite (`287 passed`); exact opt-in live `RJDI`
  route; configured-provider invariant; and concrete-live-route manifest invariant
  (`3 passed`). Strict state is now `228/345` native/live-backed and `117/345`
  fallback-only. This is periodic quarterly issuer disclosure, not a claim of daily
  coverage. The full live-provider matrix still has no completed final result.

## Thrivent priority route retry - 2026-07-16T18:15Z

- Retried current ETF and holdings paths on both `www.thriventfunds.com` and
  `fp.thriventfunds.com`. Each browser-equivalent request returned the issuer's `403`
  gate before a complete daily holdings file or route metadata became accessible.
- `thrivent` remains fallback-only and uncounted. This is an independently refreshed
  blocker, not a reason to use SEC EDGAR as a primary adapter.

## SoFi priority route retry - 2026-07-16T18:10Z

- Retried the remaining high-priority `sofi` identity with its current `SFY` product
  pages, ETF directory, sitemap, and the `/wealth/backend/api/` namespace explicitly
  allowed in its robots file. Every candidate returned the same `403` WAF document to
  a normal browser-equivalent backend request; no executable complete issuer-native
  portfolio artifact was exposed.
- `sofi` remains fallback-only and uncounted. Do not replace this finding with SEC
  EDGAR or an invented generic URL route. Continue the queue with the next reachable
  issuer, while retaining this audit to avoid repeated blind probes.

## Affiliated Managers Group native holdings route - 2026-07-16T18:00Z

- Promoted `affiliated_managers_group` through a dedicated `MUNX` adapter. AMG's
  first-party product page publishes a complete current municipal-bond portfolio table
  with CUSIP, par value, market value, and net-assets weight. The adapter validates the
  exact table schema, keeps all bond rows identifier-backed and non-symbolic, and
  classifies cash/other-assets rows as non-tradable.
- AMG's publisher edge returns a TLS-fingerprint-specific `403` to `httpx` while the
  same public request succeeds through `requests`; the adapter has a narrowly scoped
  issuer-only retry rather than any generic transport fallback.
- Validation passed: Ruff; full adapter unit suite (`286 passed`); exact opt-in live
  `MUNX` route (`1 passed`). Strict state is now `227/345` native/live-backed and
  `118/345` fallback-only. The full live-provider matrix still has no completed final
  result and must not be described as passing.

## Baird native holdings route - 2026-07-16T17:30Z

- Promoted `baird` through a dedicated adapter for the three Baird-sponsored Strategas
  ETFs: `SAMM`, `SAMT`, and `SAGP`. Strategas publishes current complete portfolios at
  a stable, fund-scoped issuer download route. The adapter filters each shared file to
  the requested ETF, retains CUSIP/SEDOL/shares/weights/date data, and preserves cash
  and currency rows without inventing tradable symbols.
- Validation passed: Ruff; the full adapter unit suite (`285 passed`); exact opt-in
  networked live `SAGP` route (`1 passed`); and `git diff --check`. Strict state is now
  `226/345` native/live-backed and `119/345` fallback-only. The full live-provider matrix
  still has no completed final result and must not be described as passing.

## Praxis native holdings route - 2026-07-16T17:15Z

- Promoted `praxis` through a dedicated adapter restricted to Praxis's verified US ETF
  catalogue: `PRXG`, `PRXV`, and `PRXI`. The issuer's public website geo-blocks
  server-side rendering, but its issuer-publisher daily CSVs are directly reachable and
  return account-scoped complete portfolios. The adapter uses only that per-Praxis,
  symbol-scoped convention; it is neither a generic URL adapter nor an SEC route.
- The adapter filters shared publisher rows to the requested account, preserves CUSIP,
  shares, value, weights, date, and cash semantics, and rejects unverified symbols.
- Validation passed: Ruff; the full adapter unit suite (`284 passed`); exact opt-in
  networked live `PRXG` route (`1 passed`); and `git diff --check`. Strict state is now
  `225/345` native/live-backed and `120/345` fallback-only. The full live-provider matrix
  still has no completed final result and must not be described as passing.

## Live-backed adapter isolation repair - 2026-07-16T16:30Z

- Corrected the remaining structural gap in the declared `224/345` native/live-backed
  set: every counted adapter now owns a `fetch_latest` entry point. ARK and Pacer use
  only verified direct publisher CSV routes; Schwab and GraniteShares now own their
  fund-page-to-file discovery and validate the publisher host; Cambria, Beyond
  Investing, Motley Fool, Tuttle, Yorkville, Groupe BPCE/Natixis, Natixis Investment
  Managers, SEI, and 3EDGE own their issuer fetch entry points while sharing only
  transport/parser utilities for genuinely identical FilePoint, Google Sheets, or
  daily-export layouts.
- Added a full registry invariant in the adapter unit suite: any configuration marked
  `live_tested_default_route=True` must instantiate an adapter whose class defines
  `fetch_latest` itself. This supplements the existing live manifest invariant that
  requires a concrete live-route test for every such key.
- Validation passed: full adapter suite (`283 passed`), Ruff, diff check; exact opt-in
  live routes passed for ARK, Pacer, Schwab, GraniteShares, Cambria, Beyond Investing,
  Motley Fool, Tuttle, Yorkville, Groupe BPCE/Natixis, Natixis Investment Managers,
  and SEI. A full `231`-test live sweep was started but did not return a final process
  result through the execution channel, so do not claim a current full-matrix pass.
- Strict state remains `224/345` native/live-backed and `121` fallback-only. Continue
  the active goal by implementing a genuine issuer-native route for the next reachable
  fallback identity; do not treat the count or the isolation repair as all-provider
  completion.
...
## Latest checkpoint - 2026-07-12T03:30Z

- Promoted `rafferty` from generated recognition-only support to native/live-backed support.
  `RaffertyHoldingsAdapter` is an explicit provider implementation for Rafferty Asset
  Management's Direxion-managed ETF suite. It uses the official per-symbol daily Direxion
  holdings export and preserves the existing issuer-specific CSV parser and asset semantics.
- Live validation symbol: `COM`, returning more than five issuer-native holdings rows.
- Registry count: `345` registered provider keys, `152` native/live-backed integrations, and
  `193` remaining. SEC EDGAR remains fallback-only and is not counted.
- Validation: full adapter unit suite `192 passed`; focused Rafferty live route `1 passed,
...
- Added an isolated `MairsPowerHoldingsAdapter` for the Mairs & Power Minnesota Municipal Bond
  ETF (`MINN`) public portfolio page. It normalizes the issuer's bare table-header markup and
  bespoke header names before parsing the complete portfolio table.
- It preserves CUSIP, par/shares, market value, weight, fixed-income classification, and issuer
  composition date without inventing equity tickers for municipal-bond positions.
- Live validation symbol: `MINN`, returning more than 20 parseable issuer-native rows.
- Registry count after promotion:
  - registered ETF provider keys: `345`
  ...
  - SEC EDGAR remains fallback only and is not counted as native provider support.
- Validation:
  - full ETF adapter unit suite: `189 passed`
  - focused live Mairs & Power route: `1 passed, 148 deselected`
  - live provider matrix: `1 passed`
  - targeted ruff and `git diff --check`: passed
- Feature commit: `ac74d49 feat(etf-holdings): add Mairs Power native holdings route`.
...
- Validation passed: targeted Ruff; full adapter unit suite (`245 passed`); live `LCF`; strict
  ...
  fallback-only.
## 2026-07-16 - Guinness Atkinson and Man Group source audit

- Guinness Atkinson's public fund-resource catalogue advertises ETF holdings documents, but direct backend access
  returns HTTP `403`; its indexed quarterly PDFs are therefore not a stable current discovery route from the app.
- Man Group's public ETF UI exposes an `All holdings` workbook for MEMA, but the verified direct issuer
  `download-holdings` endpoint returns HTTP `500` to both `httpx` and `requests`, including browser-equivalent headers.
- Neither provider was promoted. Both remain fallback-only until an executable complete issuer-native holdings route
  can be validated; SEC EDGAR remains fallback-only and uncounted.
## Dana native route - 2026-07-20T14:05Z

- Promoted `dana` through an isolated adapter for the issuer's official,
  fund-scoped daily holdings CSV. The adapter accepts only Dana's own endpoint,
  validates the requested ETF against the CSV `Account` value, and preserves ticker,
  CUSIP, shares, market value, weight, and disclosed date. Cash and fixed-income rows
  remain non-tradable.
- Dana's Cloudflare edge rejects the equivalent HTTPX request with `403`, while the
  public CSV succeeds through `requests`; that bounded transport fallback is local to
  this adapter and covered by the fixture.
- Validation passed: Ruff; full `300`-test adapter suite; concrete opt-in DANA route;
  configured-provider registry invariant; and concrete live-route manifest invariant
  (`4 passed`). Strict state is `239/345` native/live-backed and `106/345` fallback-only.
## Dawn Global native route - 2026-07-20T14:25Z

- Promoted `dawn_global` through an isolated adapter for the verified legal-parent
  relationship with Tema. It owns the Dawn Global route entry point, restricts itself
  to Tema's public symbol-scoped complete holdings CSV, records Tema as the publisher,
  and reuses only the stable canonical CSV parser.
- Validation passed: Ruff; full `301`-test adapter suite; concrete opt-in DSPY route
  (`507` rows); configured-provider registry invariant; and concrete live-route
  manifest invariant (`5 passed`). Strict state is `240/345` native/live-backed and
  `105/345` fallback-only.
## FMC Group native route - 2026-07-20T14:50Z

- Promoted `fmc_group` through an isolated FMCX adapter. It reads the official product
  page's linked complete quarterly holdings workbook, validates the restricted fund
  range and workbook schema, keeps cash non-tradable, and explicitly records the
  issuer's stated quarterly cadence and 60-day disclosure lag.
- FMC's Cloudflare edge rejects ordinary HTTPX requests but accepts the same public
  page and workbook through `requests`; that fallback is bounded to this adapter.
- Validation passed: Ruff; full `302`-test adapter suite; concrete opt-in FMCX route;
  configured-provider registry invariant; and concrete live-route manifest invariant
  (`6 passed`). Strict state is `241/345` native/live-backed and `104/345` fallback-only.

## Prospera native route - 2026-07-21T15:05Z

- Promoted `prospera` through an isolated `ProsperaHoldingsAdapter` for THRV.
  It verifies the issuer's fund-data page, its explicitly declared FilePoint portal,
  THRV portal identity, and expected complete daily CSV schema before accepting rows.
  Treasury/cash-like rows remain non-equity while CUSIP, shares, market value, weight,
  and as-of date are preserved.
- Validation passed: deterministic parser test, Ruff, configured-provider registry
  invariant, concrete-live-route manifest invariant, and exact opt-in live THRV route.
  Strict state: `257/345` native/live-backed and `88/345` fallback-only; SEC EDGAR
  remains fallback-only and uncounted.

## WisdomTree source retry - 2026-07-21T15:20Z

- Rechecked the high-priority WisdomTree route starting from WTV's current official
  product page. The public page advertises a full-holdings action, but direct
  browser-equivalent backend retrieval receives a Cloudflare `403` challenge before
  the action's issuer-native artifact can be traced or verified.
- WisdomTree therefore remains fallback-only and uncounted. No generic parser or
  SEC fallback was substituted for primary provider support; continue with the
  remaining issuer queue.

## Remaining issuer audit - 2026-07-21T17:42Z

- Revalidated three more fallback-only candidates rather than promoting them on
  search-index evidence. Q3 Tactical's official domain returns an issuer-side
  Cloudflare `503` with `Retry-After: 3600`; therefore its complete holdings
  artifact cannot be discovered or live-tested now. Westwood's official MDST
  CSV and Ridgeline's official ACVF holdings page remain Cloudflare-gated from
  backend retrieval.
- Sterling Capital itself is already natively supported. The distinct
  `sterling_fund` identity has not yielded a U.S. issuer-native holdings route,
  so it remains fallback-only rather than being incorrectly merged or counted
  through the Sterling Capital integration.
- Current working-tree count remains `276/345` native/live-backed and `69/345`
  fallback-only. The last implementation commit is `260/345`; the 16-provider
  expansion is still uncommitted application and test work.

## Priority issuer route retry - 2026-07-21T17:50Z

- SoFi's official ETF pages are discoverable and publish current top-ten data,
  but direct backend retrieval of the SFYF page receives a Cloudflare managed
  challenge. No complete official holdings artifact was available from that
  request, so SoFi remains fallback-only rather than being promoted from a
  partial portfolio table.
- Q3 Tactical's official domain independently returned Cloudflare HTTP `503`
  with a one-hour retry window. The provider cannot currently furnish an
  executable complete holdings route for a native integration or live test.

## Anfield live-route revalidation - 2026-07-21T17:54Z

- Rechecked the previously implemented Anfield `AEMS` product-page/CSV parser
  against its current official page after indexed content showed a current
  holdings download. The production `httpx` path receives HTTP `404`; a second
  direct browser-header request to both canonical and `www` hostnames receives
  issuer-side HTTP `403`. No backend-retrievable complete CSV could therefore
  be live-tested.
- A temporary native/live-backed promotion and exact `AEMS` live test were
  intentionally reverted after that failed test. Anfield remains fallback-only;
  the strict state is unchanged at `276/345` native/live-backed and `69/345`
  fallback-only. The focused deterministic parser test still passes.

## Current native-provider validation - 2026-07-21T18:00Z

- Full deterministic adapter coverage passed: `344 passed`.
- The opt-in provider registry and concrete-live-route manifest invariants
  passed (`2 passed`), proving all currently counted native providers declare
  a concrete live test route.
- A complete `RUN_LIVE_ETF_HOLDINGS_TESTS=1` sweep was started for the current
  `276` native providers, but the execution channel ended after partial progress
  and without pytest's footer. It is deliberately not recorded as a successful
  full live sweep; rerun it in a durable runner before using it as broad evidence.

## CCM native route - 2026-07-21T18:10Z

- Promoted `ccm` through an isolated OWNS adapter after verifying that Community
  Capital Management's own product page links its current complete holdings CSV.
  The adapter accepts only the verified OWNS fund CUSIP/name, preserves the
  issuer's fixed-income and cash rows without inventing equity symbols, and
  records the source as a fund-scoped current issuer export.
- CCM's Cloudflare edge rejects `httpx` but returns the exact same public CSV to
  `requests`; that retry is deliberately local to this known CCM route.
- Validation passed: focused deterministic parser test; full `345`-test adapter
  suite; Ruff; exact opt-in OWNS live route; provider registry invariant; and
  concrete-live-route invariant. Strict working-tree state is `277/345`
  native/live-backed and `68/345` fallback-only.

## Native-provider reliability sweep - 2026-07-21T19:10Z

- A full opt-in `285`-test live-provider matrix completed but did not pass:
  VanEck SMH initially timed out, ETF Architect QVAL received an issuer-side
  `502`, and Virtus SSMG timed out on its HTML product page. VanEck now has a
  bounded adapter-local timeout retry, covered by a deterministic fixture and
  a focused live SMH rerun.
- Virtus was corrected to use its stable issuer-hosted direct SSMG positions
  workbook rather than requiring the fragile product page. The direct XLS route
  passed both deterministic and focused live validation. Its binary result now
  records a tabular raw-text representation, satisfying the shared audit result
  contract without fabricating an HTML source.
- ETF Architect QVAL remains an unresolved native-route reliability defect: its
