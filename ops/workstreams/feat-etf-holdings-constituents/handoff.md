...
- Current code-derived state: 496 registered, 369 native/live-backed, 127
  fallback-only.
- Current fallback status split: 8 access-blocked, 110 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/369/127 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `1aa7cc48` (Castellan native CTEF/CTIF
...
- Product implementation is underway; current changes add Guggenheim, ARS,
  ...
  Bushido, CapForce, and Castellan
...
advanced to 369/127, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, Blueprint, Bridgeway, Brookstone, BufferLABS, Bushido, CapForce, and Castellan promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, `bridgeway`, `brookstone`, `bufferlabs`, `bushido`, `capforce`, and `castellan` are native-promoted; `advisors_asset_management` and `amplius` are
...
The audit ledger currently has 100 queued fallback records still requiring
...
route evidence, starting with the ranked queue after Castellan at `conductor_fund`, and checkpoint
