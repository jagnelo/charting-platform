...
- Current code-derived state: 496 registered, 368 native/live-backed, 128
  fallback-only.
- Current fallback status split: 8 access-blocked, 111 discovery, 3
...
- Latest implementation checkpoint: `992a554d` (CapForce native FFTY/BOUT
...
advanced to 368/128, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, Blueprint, Bridgeway, Brookstone, BufferLABS, Bushido, and CapForce promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, `bridgeway`, `brookstone`, `bufferlabs`, `bushido`, and `capforce` are native-promoted; `advisors_asset_management` and `amplius` are
...
route evidence, starting with the ranked queue after CapForce at `castellan`, and checkpoint
