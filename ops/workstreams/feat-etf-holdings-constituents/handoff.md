...
- Current code-derived state: 496 registered, 364 native/live-backed, 132
  fallback-only.
- Current fallback status split: 8 access-blocked, 115 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/364/132 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `c1caf555` (Bridgeway native BBLU/BAGX/BRSV/
  BSVO/BUSM holdings routes and live coverage); prior Blueprint checkpoint
...
- Product implementation is underway; current changes add Guggenheim, ARS,
  Avory, Ballast, Bancreek, BeeHive, Blueprint, and Bridgeway
...
advanced to 364/132, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, Blueprint, and Bridgeway promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, and `bridgeway` are native-promoted; `advisors_asset_management` and `amplius` are
...
The audit ledger currently has 105 queued fallback records still requiring
...
route evidence, starting with the ranked queue after Bridgeway at `brookstone`, and checkpoint
