...
- Current code-derived state: 496 registered, 370 native/live-backed, 126
  fallback-only.
- Current fallback status split: 8 access-blocked, 109 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/370/126 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `fd07b17f` (Conductor native CGV declared
...
- Product implementation is underway; current changes add Guggenheim, ARS,
  ...
  Bushido, CapForce, Castellan, and Conductor
...
advanced to 370/126, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, Blueprint, Bridgeway, Brookstone, BufferLABS, Bushido, CapForce, Castellan, and Conductor promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, `bridgeway`, `brookstone`, `bufferlabs`, `bushido`, `capforce`, `castellan`, and `conductor_fund` are native-promoted; `advisors_asset_management` and `amplius` are
