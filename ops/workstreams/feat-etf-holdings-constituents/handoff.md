...
- Current code-derived state: 496 registered, 360 native/live-backed, 136
  fallback-only.
- Current fallback status split: 8 access-blocked, 119 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/360/136 snapshot; future updates must remain code-derived.
...
advanced to 360/136, including the Guggenheim, ARS, Avory, and Ballast promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, and `ballast` are native-promoted; `advisors_asset_management` and `amplius` are
...
The remaining 127 current fallbacks still require
