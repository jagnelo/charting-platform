...
- Current code-derived state: 496 registered, 375 native/live-backed, 121
  fallback-only.
- Current fallback status split: 8 access-blocked, 104 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/375/121 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `1b29a6fb` (Everence/Praxis PRXG, PRXV, and PRXI native routes; Even Herd native EHLS route; the existing
...
advanced to 375/121, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, `bridgeway`, `brookstone`, `bufferlabs`, `bushido`, `capforce`, `castellan`, `conductor_fund`, `cresalta`, `elm`, `esoterica`, `even_herd`, and `everence` are native-promoted; `advisors_asset_management` and `amplius` are
...
Azure CSV convention. The audit ledger currently has 86 queued fallback records still
...
provenance and exchange-aware parsing. Continue replacing baseline placeholders
with first-party route evidence, starting with `falconx`, and checkpoint
