...
- Current code-derived state: 496 registered, 361 native/live-backed, 135
  fallback-only.
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/361/135 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `b4c96335` (Ballast native holdings route);
  prior Avory/ARS checkpoints are `695dea16`/`f33224ab` and the Guggenheim audit
  receipt remains `c4bef2ec`.
- Product implementation is underway; current changes add Guggenheim, ARS,
  Avory, Ballast, and Bancreek
...
advanced to 361/135, including the Guggenheim, ARS, Avory, Ballast, and Bancreek promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, and `bancreek` are native-promoted; `advisors_asset_management` and `amplius` are
...
The remaining 126 current fallbacks still require
