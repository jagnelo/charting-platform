...
- Current code-derived state: 496 registered, 373 native/live-backed, 123
  fallback-only.
- Current fallback status split: 8 access-blocked, 106 discovery, 3
  ...
  Elm and Esoterica are no longer runtime fallbacks after their native
  promotions).
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/373/123 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `915282cd` (Esoterica native WUGI route; the existing
...
advanced to 373/123, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, Blueprint, Bridgeway, Brookstone, BufferLABS, Bushido, CapForce, Castellan, Conductor, CresAlta, Elm, and Esoterica promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, `blueprint`, `bridgeway`, `brookstone`, `bufferlabs`, `bushido`, `capforce`, `castellan`, `conductor_fund`, `cresalta`, `elm`, and `esoterica` are native-promoted; `advisors_asset_management` and `amplius` are
