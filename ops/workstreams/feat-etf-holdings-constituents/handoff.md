...
- Current code-derived state: 496 registered, 363 native/live-backed, 133
  fallback-only.
- Current fallback status split: 8 access-blocked, 116 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/363/133 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: pending Blueprint changeset (Blueprint native
  TFPN holdings route); prior BeeHive checkpoint `bc77c274` (BeeHive native holdings route
  plus promoted-adapter contract alignment);
...
- Product implementation is underway; current changes add Guggenheim, ARS,
  Avory, Ballast, Bancreek, BeeHive, and Blueprint
...
advanced to 363/133, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
BeeHive, and Blueprint promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, `beehive`, and `blueprint` are native-promoted; `advisors_asset_management` and `amplius` are
...
The audit ledger currently has 106 queued fallback records still requiring
...
route evidence, starting with the ranked queue after Blueprint, and checkpoint
