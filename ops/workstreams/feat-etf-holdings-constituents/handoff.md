...
- Current code-derived state: 496 registered, 362 native/live-backed, 134
  fallback-only.
- Current fallback status split: 8 access-blocked, 118 discovery, 3
...
- `docs/etf-provider-universe.md` has been reconciled from code to the current
  496/362/134 snapshot; future updates must remain code-derived.
...
- Latest implementation checkpoint: `bc77c274` (BeeHive native holdings route
...
- Product implementation is underway; current changes add Guggenheim, ARS,
  Avory, Ballast, Bancreek, and BeeHive
...
advanced to 362/134, including the Guggenheim, ARS, Avory, Ballast, Bancreek,
and BeeHive promotions. Continue current gaps; do not recreate completed work or
...
`guggenheim`, `ars`, `avory`, `ballast`, `bancreek`, and `beehive` are native-promoted; `advisors_asset_management` and `amplius` are
...
The remaining 125 current fallbacks still require
issuer-specific evidence and
final dispositions. Continue replacing baseline placeholders with first-party
route evidence, starting with the ranked queue after BeeHive, and checkpoint
