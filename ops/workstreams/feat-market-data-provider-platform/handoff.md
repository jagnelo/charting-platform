...
- Current source checkpoint: the current uncommitted boundary adds explicit IBKR futures-history capability through the documented generic conid path. Before this boundary, the latest committed checkpoint was `67dfa38d4`; the branch's latest authoritative backend gate passed `2,445/2,445` at `81.60%`, and the newly configured Alpaca/MarketData.app live revalidation passed `7/7`. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
