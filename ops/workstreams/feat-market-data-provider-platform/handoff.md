...
- Current source checkpoint: `7ef9e8ec8` carries the latest backend evaluator hardening after the repaired migration graph and live-provider evidence. The branch now reports the 47-case/41-pass live matrix, the `2,433/2,433` backend gate at `81.52%`, one Alembic head (`bc2d3e4f5a6b`), and passed staging-baseline migration compatibility. Heatmap evaluation is DB-first with bounded per-instrument sparkline reads; historical entries below that predate the current source checkpoint and remain preserved as evidence. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, and coordinator review before promotion.
