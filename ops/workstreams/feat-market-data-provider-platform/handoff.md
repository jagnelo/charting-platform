...
- Current source checkpoint: `43a7d8f091ef` is the latest verified source state. It preserves provider-specific MarketData.app candle accounting, models the distinct Starter/Trader trial plan contracts with explicit expiry enforcement and safe blank-environment parsing, reports complete invalid trial controls, separates tokenized historical-price capability, and uses an explicit provider identifier in the tokenized-history availability probe. Focused provider/quota coverage and the authoritative backend gate pass; the full gate passes `2,455/2,455` at `81.68%` with 89 warnings. No frontend or ETF-provider adapter files changed. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
