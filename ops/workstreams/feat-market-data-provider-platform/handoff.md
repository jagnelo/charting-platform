...
- Current source checkpoint: the current boundary adds raw-only adjustment handling to provider availability probes after the explicit IBKR futures-history capability. The latest committed source before this boundary was `39034ac93`; the authoritative backend gate now passes `2,447/2,447` at `81.63%`, and the newly configured Alpaca/MarketData.app live revalidation remains `7/7`. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
