...
- Current source checkpoint: the current boundary hardens tokenized historical persistence after the raw-only availability-probe fix. Tokenized history is now tested through the real canonical persistence boundary (raw `24_7` bar, scoped `MarketSeries`/default, observation, and dataset state), and observation upserts use the declared unique columns as a portable conflict target. Focused tokenized coverage is `23/23`; the latest authoritative backend gate passes `2,450/2,450` at `81.66%` with 89 warnings. The newly configured Alpaca/MarketData.app live revalidation remains `7/7`. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
