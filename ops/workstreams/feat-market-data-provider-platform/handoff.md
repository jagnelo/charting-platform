...
- Current source checkpoint: `91d63a5fc` makes MarketData.app stock-candle accounting provider-specific. Its documented one-credit-per-1,000-candles charge is estimated from the inclusive calendar-date request (with a conservative full-day upper bound for intraday resolutions) and passed into explicit/latest/bulk history reservations; fixed one-credit fallback entries were removed for those response-priced operations, so unsupported resolutions remain fail-closed. Focused provider/runtime/service coverage passes `244/244`; the authoritative backend gate passes `2,452/2,452` at `81.67%` with 89 warnings. No external provider calls or credentials were used for this source change. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
