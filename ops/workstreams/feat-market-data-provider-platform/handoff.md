...
- Current source checkpoint: `7b1102017900` is the latest verified source state. It preserves provider-specific MarketData.app candle accounting, models the distinct Starter/Trader trial plan contracts, separates tokenized historical-price capability, and uses an explicit provider identifier in the tokenized-history availability probe. Focused provider/quota coverage and the authoritative backend gate pass; the full gate passes `2,454/2,454` at `81.67%` with 89 warnings. No frontend or ETF-provider adapter files changed. The remaining gates are provider/legal/deployment/shadow, future-evaluator coordination, production NMS/OTC reconciliation, IBKR gateway evidence, and coordinator review before promotion.
