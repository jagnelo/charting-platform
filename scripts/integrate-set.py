...
def main() -> int:
    ...
    with integrate.integration_lock(repo):
        ...
        for branch in args.branches:
            ...
            sources.append(
                (branch, source, integrate.out(["git", "rev-parse", "HEAD"], source))
            )
        frozen_sources = "\n".join(f"{b}:{s}" for b, _, s in sources)
        label = "batch-" + "-".join(
            branch.replace("/", "-") for branch, _, _ in sources
        )
        ...
        if candidate.exists():
            raise SystemExit(
                f"candidate already exists; inspect or remove it explicitly: {candidate}"
            )
        ...
        integrate.run(
            ["git", "worktree", "add", "--detach", str(candidate), master_sha], repo
        )
        try:
            for branch, _, sha in sources:
                merged = integrate.run(
                    [
                        "git",
                        "merge",
                        "--no-ff",
                        sha,
                        "-m",
                        f"Integrate {branch} at {sha}",
                    ],
                    candidate,
                    check=False,
                )
                if merged.returncode:
                    conflicts = integrate.out(
                        ["git", "diff", "--name-only", "--diff-filter=U"], candidate
                    )
                    raise SystemExit(
                        f"batch conflict while integrating {branch}: {conflicts}"
                    )
            for branch, source, sha in sources:
                if integrate.out(["git", "rev-parse", "HEAD"], source) != sha:
                    raise SystemExit(
                        f"source branch advanced during batch preparation: {branch}"
                    )
                integrate.run(
                    ["make", "branch-tests", f"INTEGRATION_BRANCH={branch}"], candidate
                )
            ...
            raise
        if integrate.out(["git", "rev-parse", "HEAD"], repo) != integrate.out(
        ...
        print(
            json.dumps(
                {
                    "branches": [b for b, _, _ in sources],
                    "candidate": str(candidate),
