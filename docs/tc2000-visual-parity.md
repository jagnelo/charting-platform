...
aligned range centers, with Boolean column/filter/scan/gauge/alert fan-out. No authoritative V25
capture represents this promotion-row composition, so exact typography, placement, spacing, and
post-save states remain `required_missing`; no screenshot baseline, mask, threshold, skip, or visual
acceptance rule changed.

The exact-tip gate at metadata tip `93883674` after formatter correction `c2f3a588` (product tip
`7f672897`) passed all non-visual stages and functional Playwright (`157` passed, `106` documented
skips across `263` specs). The unchanged four-project matrix completed `104` cases with `98` passes
and six failures: `watchlist-column-editor-open` at visual-1080p-100/125 (`13,844` pixels each),
and `workspace-floating` at visual-1080p-100/125/1440p-100/125
(`11,901`/`11,901`/`12,097`/`9,770` pixels). Teardown and resource accounting were clean; these
state-oracle mismatches remain the only visual gate failures, alongside the broader canonical
...
Top-level and nested group frames/counts are rendered as non-interactive HTML overlays for ordinary
tile maps and drawn directly into the large-universe canvas. This closes a functional hierarchy visibility
defect in the arbitrary-universe map, not the visual reference gap. Exact V25 group-label placement,
borders/gutters, density, text scaling, hover treatment, and root-versus-drill-down composition
remain `required_missing` in the reference board. Focused layout/Market Map coverage is `35/35`,
...
gap for final visual baselines remains open; direct map-to-breadth/Study-Lab publication,
persisted-cache status, and named snapshot status are represented by the current interim oracle.

## 2026-08-17 — Named Market Map snapshot visual state

The interim Market Map oracle now includes a dense snapshot selector with live/cached state,
user-authored name input, save action, restore selection, active snapshot labelling, and delete
action. Component coverage verifies the lifecycle and user-visible status while the restored map
retains source, coverage, freshness, warnings, and tile metrics.

The visual reference board has no authoritative Version 25 capture for this exact named-snapshot
