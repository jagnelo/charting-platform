...
- [x] Keep the universal source-polymorphic Market Map usable for large locked index/ETF
      constituent sets, personal/combo watchlists, and explicit symbol sets. Above 1,500 valid
      cells the map now paints one deterministic canvas instead of creating one Vue button per
      member; geometry, colour, selection outline, hover details, zoom, and pan remain driven by
      the same canonical batch response and layout model.
- [x] Prevent a drag-to-pan gesture from becoming an accidental member selection. Add a bounded
      keyboard-accessible symbol/name search for canvas maps so large universes remain selectable
...
- [ ] The reference board has no authoritative V25 large-universe treemap capture, canvas/text
...
- [ ] Normalize those inputs behind one `WatchlistSource` contract. Index and ETF universes are
      system-managed, locked watchlists: users may follow/pin/select them, use them in maps,
      grids, scans, breadth, alerts, and linked charts, or clone a dated snapshot, but may not
      silently edit their membership. Membership refreshes create a new version with source,
      effective-at, known-at, composition date, and coverage lineage; this is distinct from a
      screener-managed watchlist whose membership is generated by a scan.
...
- [x] Support 1D, 1W, MTD, YTD, 1M, 3M, 6M, 1Y, and arbitrary completed-session periods with
      explicit adjustment and session semantics. The workstation exposes CUSTOM start/end dates
      and persists them in the serializable tool configuration.
...
- [x] The workstation exposes a CUSTOM completed-session period with explicit start/end dates;
      those dates flow through the server batch/cache identity and survive layout configuration
