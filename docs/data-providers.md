...
| FINRA OTC directory | current `otcSecurityMaster` DAPI snapshot, or configured pipe-delimited OTC/OTCBB mirror | `FINRA_OTC_SYMBOL_DIRECTORY_URL` | Official FINRA synchronous platform ceiling: 1,200 requests/minute/IP and 3 MB maximum response; source-specific polling and redistribution terms still require review | configured source / rolling IP request window | full current DAPI pagination is live-proven; response-dependent partition/page cost is explicitly unknown and fail-closed, so the adapter remains non-routable until a reviewed bound/profile and source terms are approved |
| FRED | macro/rates/FX daily series | `FRED_API_KEY` | FRED v1 errors document that rate limiting can return HTTP 429 but do not publish a numeric ceiling or enforcement scope; the terms permit provider-adjusted limits plus series-specific copyright/redistribution restrictions. The separate v2 2-requests/second rule is not applied to this v1 adapter | API key / provider-defined | **numeric limit and scope unknown; non-routable until the v1 contract and terms are reviewed** |
...
| Marketstack | daily EOD history and ticker discovery | `MARKETSTACK_API_KEY` | Free-plan pricing publishes 100 requests/month and one year of history; a stale FAQ sentence says 1,000, so the checked-in contract uses the lower 100-request ceiling; EOD responses expose 100-row pagination metadata | key / calendar month | history follows returned pagination and reserves a conservative page count before execution; daily history live-proven |
| EODHD | long-history daily EOD, fundamentals/profile, US exchange list | `EODHD_API_KEY` | Free 20 API calls/day and 1,000 requests/minute; EOD free history is limited to one year; data-heavy endpoints consume multiple calls (fundamentals/options 10, intraday/technical/news 5) | API key / minute requests + GMT calendar-day call budget | daily history live-proven; operation costs are explicit |
...
states that rate limiting can return HTTP 429, but it does not publish a
numeric ceiling or enforcement scope. The [v1 API terms](https://fred.stlouisfed.org/docs/api/terms_of_use.html)
...
not apply that v2 rule to the v1 adapter. The runtime records all three
unknown dimensions explicitly and keeps FRED non-routable until the v1
contract, provider-adjustable limits, and series rights are reviewed. The
