...
| Tiingo | EOD history, search, profiles | `TIINGO_API_KEY` | 500 unique symbols/month, 50/hour, 1,000/day, 1 GB/month (free Starter) | API key / multiple windows | EOD live-proven; **not routable until bandwidth accounting exists** |
...
| FMP | stable-API daily history, profile, stock list | `FMP_API_KEY` | observed free account 250 calls/day and 512 MB/30 days | key / provider day + rolling/account bandwidth | stable EOD history live-proven; **not routable until bandwidth accounting exists** |
