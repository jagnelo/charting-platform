...
| FINRA | consolidated short interest (OAuth Query API) and OTC Daily List lifecycle/corporate-action deltas | `FINRA_CLIENT_ID`, `FINRA_CLIENT_SECRET` | 1,200 synchronous requests/minute/IP; max 5,000 records and 3 MB per synchronous response; public credential capped at 10 GB downloaded/month | OAuth client / IP + calendar-month credential bandwidth | both OAuth datasets live-proven; response bytes are now observed and durable, but **not routable until byte-budget reservation/enforcement exists** |
