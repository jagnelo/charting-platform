...
| FINRA | consolidated short interest (OAuth Query API) and OTC Daily List lifecycle/corporate-action deltas | `FINRA_CLIENT_ID`, `FINRA_CLIENT_SECRET` | 1,200 synchronous requests/minute/IP; 20 asynchronous requests/minute/dataset/account; max 5,000 records and 3 MB per synchronous response; public credential capped at 10 GB downloaded/month | OAuth client / IP + dataset/account + calendar-month credential bandwidth | both synchronous datasets live-proven; conservative 3 MB pre-reservation settles to observed bytes; asynchronous dataset submission is not implemented |
...
| Tiingo | EOD history, search, profiles | `TIINGO_API_KEY` | 500 unique symbols/month, 50/hour, 1,000/day, 1 GB/month (free Starter) | API key / multiple windows | EOD live-proven; response bytes are now observed and durable, but **not routable until byte-budget reservation/enforcement exists** |
...
| FMP | stable-API daily history, profile, stock list | `FMP_API_KEY` | observed free account 250 calls/day and 512 MB/30 days | key / provider day + rolling/account bandwidth | stable EOD history live-proven; response bytes are now observed and durable, but **not routable until byte-budget reservation/enforcement exists** |
