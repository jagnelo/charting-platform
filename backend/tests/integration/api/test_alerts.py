"""
Integration tests for /alerts/price and /alerts/indicator endpoints.
"""
...
class TestPriceAlerts:
    def test_list_alerts_empty(self, client, auth_headers):
        res = client.get("/api/v1/alerts/price", headers=auth_headers)
        assert res.status_code == 200
        assert res.json() == []

    def test_list_alerts_requires_auth(self, client):
        res = client.get("/api/v1/alerts/price")
    ...
    def test_create_alert_user_isolation(self, client, db, instrument):
        ...
        # Verify user_b's alert appears in their own list
        res_b = client.get("/api/v1/alerts/price", headers=headers_b)
    ...
    def test_delete_alert(self, client, auth_headers, price_alert):
        res = client.delete(f"/api/v1/alerts/price/{price_alert.id}", headers=auth_headers)
        assert res.status_code == 204

        # Verify gone
        res2 = client.get("/api/v1/alerts/price", headers=auth_headers)
    ...
    def test_delete_alert_wrong_user(self, client, db, price_alert):
        ...
        res = client.delete(f"/api/v1/alerts/price/{price_alert.id}", headers=headers)
    ...
    def test_rearm_triggered_alert(self, client, auth_headers, db, price_alert):
        ...
        res = client.post(f"/api/v1/alerts/price/{price_alert.id}/rearm", headers=auth_headers)
        assert res.status_code == 200
    ...
    def test_list_alerts_by_instrument(self, client, auth_headers, instrument, instrument_b):
        ...
        res = client.get(
            "/api/v1/alerts/price", params={"instrument_id": instrument.id}, headers=auth_headers
        )
...
class TestIndicatorAlerts:
    def test_create_indicator_alert_vs_value(self, client, auth_headers, instrument):
        ...
        assert res.status_code == 201
        data = res.json()
        assert data["indicator_a_type"] == "rsi"
    ...
    def test_create_indicator_alert_cross(self, client, auth_headers, instrument):
        ...
        res = client.post(
            ...
            json={
                "instrument_id": instrument.id,
                "timeframe": "D1",
                "indicator_a_type": "ema",
                "indicator_a_params": {"period": 50},
                "condition": "crosses_above",
                "indicator_b_type": "ema",
                "indicator_b_params": {"period": 200},
            },
        )
        assert res.status_code == 201
        data = res.json()
        assert data["indicator_b_type"] == "ema"
    ...
    def test_list_indicator_alerts(self, client, auth_headers, instrument):
        ...
        res = client.get("/api/v1/alerts/indicator", headers=auth_headers)
        assert res.status_code == 200
    ...
    def test_indicator_alert_missing_threshold_and_b_type_fails(self, client, auth_headers, instrument):
        ...
        res = client.post(
            ...
            json={
                ...
                "indicator_a_params": {"period": 14},
                "condition": "crosses_below",
                # no threshold_value, no indicator_b_type
            },
        )
