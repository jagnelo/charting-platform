...
class CodeAsset(Base, TimestampMixin):
    ...
    versions: Mapped[list[CodeVersion]] = relationship(back_populates="asset", cascade="all, delete-orphan")


class CodeVersion(Base, TimestampMixin):
    __tablename__ = "code_version"
    __table_args__ = (UniqueConstraint("code_asset_id", "version_number", name="uq_code_version_number"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    code_asset_id: Mapped[int] = mapped_column(ForeignKey("code_asset.id", ondelete="CASCADE"), index=True)
...
class ResearchRun(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("user.id", ondelete="CASCADE"), index=True)
    code_version_id: Mapped[int] = mapped_column(ForeignKey("code_version.id", ondelete="RESTRICT"), index=True)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default=ResearchRunStatus.QUEUED.value)
    ...
    artifacts: Mapped[list[ResearchArtifact]] = relationship(back_populates="run", cascade="all, delete-orphan")
...
class ResearchArtifact(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    research_run_id: Mapped[int] = mapped_column(ForeignKey("research_run.id", ondelete="CASCADE"), index=True)
