...
async def create_watchlist(body: WatchlistCreate, db: AsyncSession = Depends(get_db)):
    wl = Watchlist(**body.model_dump())
    db.add(wl)
    await db.flush()
    await db.refresh(wl)
    return WatchlistRead(
        id=wl.id, name=wl.name, description=wl.description, created_at=wl.created_at, items=[]
    )


@router.post("/{watchlist_id}/items", response_model=WatchlistItemRead)
async def add_item(
    watchlist_id: int, body: WatchlistItemCreate, db: AsyncSession = Depends(get_db)
):
...
async def remove_item(watchlist_id: int, item_id: int, db: AsyncSession = Depends(get_db)):
