...
def execute_job(job: dict) -> dict:
    ...
    if not validation.valid:
        return {"status": "failed", "diagnostics": [item.__dict__ for item in validation.diagnostics]}
    ...
    safe_globals = {"__builtins__": {}, "output": _Output(outputs), "dataset": job.get("dataset", {})}
    _limit_resources()
